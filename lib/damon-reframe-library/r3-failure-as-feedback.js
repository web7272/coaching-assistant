...
export const R3_VARIANTS = Object.freeze({
  ...
  R3_C: 'scientist_mindset',              // ⭐ Step 7 PR-7b — errata v0.2 Patch 5.2
});
...
學員 surface failure 後給空間、不每次都翻成 feedback.`;
  ...
  // ⭐ Step 7 PR-7b — R3_C 科學家精神 vs 受審判的犯人 (errata v0.2 Patch 5.2 逐字).
  //   trigger: R3 invoke 後、學員仍把失敗連到自我評判 / 完美主義 trap 處理失敗後.
  if (variant === R3_VARIANTS.R3_C) {
    return `[SYSTEM INJECT — R3_C 科學家精神 vs 受審判的犯人 (errata v0.2 Patch 5.2)]

mode: ${mode}.
trigger: R3 invoke 後學員仍把失敗連到自我評判 / 完美主義 trap 處理失敗後.

Damon evidence:
- 「將自己從受審判的犯人轉變為實驗室的科學家」
- 「科學家不會因為實驗沒得到預期結果就覺得自己沒價值」
- 「他們會說: 太棒了、我排除了一個行不通的路徑」

話術 (errata v0.2 Patch 5.2 逐字 4 step):

**Step 1 — introduce distinction**:
> 「我聽到你在判決自己。我想說個區分——」

**Step 2 — the two modes**:
> 「你現在站的位置、有兩個選擇:
>
>  一個是『受審判的犯人』——
>  每件事、每個結果、都在問『我是不是有罪』。
...
> 「同樣的 ${failureEvent}、
>  犯人會說『我又搞砸了』、
>  科學家會說『太棒了、我排除了一條行不通的路徑、現在我離答案更近了』。
>
>  你想用哪個視角看?」
...
> 「對。
>  科學家不會因為實驗結果而質疑自己的價值——
>  因為價值不在結果裡、價值在『繼續實驗的勇氣』裡。」

cascade_to: R8 T.O.T.E. step Operate (進下一個 cycle、若 future_pacing mode active).

機制:
- 是 R3 變體、不是獨立 reframe — 通常 R3_A 之後 cascade 過來.
- mode_applicability: identity_anchoring ✓ / integration ✓✓ / future_pacing ✓.`;
  }
...
export function shouldInvoke(state, signal) {
  ...
  let variant = R3_VARIANTS.R3_A;
  // R3_B → negative generalization 又/總是/永遠 (cascade R11).
  // R3_C → R3_A 已 invoke 但學員仍 self-judge (scientist mindset 變體 cascade).
  if (signal?.negative_generalization) variant = R3_VARIANTS.R3_B;
  else if (signal?.r3_cascade_to_scientist || priorR3 >= 1) variant = R3_VARIANTS.R3_C;

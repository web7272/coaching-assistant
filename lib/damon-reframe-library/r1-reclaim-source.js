...
export function buildInject(ctx = {}) {
  ...
  // R1_C disabled by default per library §2.5 — crisis variant requires explicit
  // clinical review. Handler MUST surface this guard, not just no-op.
  if (variant === R1_VARIANTS.R1_C) {
    return `[SYSTEM INJECT — R1_C Reclaim Source (crisis variant, ⚠️ disabled)]

R1_C 是 crisis mode + passive death wish context 的特殊變體 ("[他/她] 不在我就沒動力活下去").
ship 前臨床 review 條件未通過 → 不單獨 invoke、必須先走 crisis SOP (Step 6).

本 turn 退到 crisis flow handler、不 inject R1 話術.`;
  }

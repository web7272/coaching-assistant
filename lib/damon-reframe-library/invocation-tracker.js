...
/**
 * 6/12 — Sugar for the live-engine invocation pattern:
 *   1. Build entry from {reframe_id, state, ctx}.
 *   2. Append to state's history (returns shallow patch slice).
 *
 * Centralizes the field plumbing so each invocation site is one-liner.
 *
 * Required from caller:
 *   - reframe_id (must be in REFRAME_REGISTRY)
 *   - state.turn_count_this_session  (number; falls back to 0 — buildInvocationEntry
 *     validates non-negative number, 0 acceptable)
 *   - ctx.now_iso  (ISO timestamp from caller — pure functions can NOT call Date.now)
 *   - ctx.session_id  (Optional. Falls back to null.)
 *   - state.primary_mode  (Optional. Falls back to null.)
 *
 * Optional:
 *   - variant — defaults to null (handler may pass 'standard' / 'E' / etc.)
 *
 * @param {object} args
 * @param {string} args.reframe_id
 * @param {object} args.state
 * @param {object} args.ctx
 * @param {string} [args.variant]
 * @returns {object} patch slice {reframe_invocation_history_in_session: [...prior, entry]}
 */
export function buildInvocationPatch({ reframe_id, state, ctx, variant = null }) {
  const entry = buildInvocationEntry({
    reframe_id,
    invoked_at_turn: state?.turn_count_this_session ?? 0,
    session_id: ctx?.session_id ?? null,
    outcome: 'pending',
    variant,
    mode: state?.primary_mode ?? null,
    invoked_at: ctx?.now_iso,    // caller-supplied ISO; undefined → entry.invoked_at undefined (detector doesn't use)
  });
  return appendToSessionHistoryPatch(state, entry);
}

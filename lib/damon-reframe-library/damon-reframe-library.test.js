...
test('REFRAME_REGISTRY: 8 entries (R1-R7 + R11), no R8/R9/R10/R12', () => {
  const ids = Object.keys(REFRAME_REGISTRY).sort();
  assert.deepEqual(ids, ['R1', 'R11', 'R2', 'R3', 'R4', 'R5', 'R6', 'R7']);
  assert.equal(REFRAME_REGISTRY.R8, undefined, 'R8 belongs to PR-7b');
...
test('🛑 R1 buildInject: R1_C crisis variant → 顯式 disabled guard, not the script', () => {
  const txt = R1.buildInject({ variant: R1_VARIANTS.R1_C });
  assert.match(txt, /R1_C.*disabled/);
  assert.match(txt, /ship 前臨床 review 條件未通過/);
  assert.doesNotMatch(txt, /Step 1 — pause and notice/, 'must NOT inject full script in C mode');
});

...
// ─── ⭐ 6/7 P0 Crisis output prohibitions (Vivi 6/7, A016 production smoke) ─
//
// Canonical text embedded in EVERY crisis-mode AI-facing inject:
//   - prompt_content_active_si_explicit (deep-signal-detector.js)
//   - getHandoffInject variants (handoff-three-options.js)
//   - getReminderInject variants (landing-page-reminder.js)
//   - mode-aware-router-reference.js cached §3 crisis-override section
//
// Two production-observed AI improvisation failure modes this locks:
//   1. AI quoted student's「自殺」 word back verbatim. Vivi: AI must NEVER
//      emit「自殺」 in output. Use neutral noun-phrases instead.
//   2. Post active-SI-confirmed, AI re-asked SI assessment instead of advancing
//      to Step 6. de939b6 fast-path skips Step 1/2/3 for the FIRST turn; this
//      reminder locks it for subsequent turns too.
//
// Defense 1 (prompt rule) + Defense 2 (lib/util/crisis-output-scrubber.js
// deterministic backstop) are belt-and-suspenders. Prompts lower probability;
// scrubber guarantees zero「自殺」 reach the student.
export const CRISIS_OUTPUT_PROHIBITIONS = `⚠️ Crisis 輸出禁令 (Vivi 6/7 P0 safety):

❌ AI 回應中絕不出現「自殺」二字 — 即使學員自己這樣說了, AI 回覆改用中性詞:
   「這件事」/「你說的」/「這個當下你的狀態」/「剛才講的」. 不要 quote echo
   學員原話的「自殺」.

❌ active SI 已 confirmed → 後續輪不再問 SI / 念頭 / 計畫. 全部禁止:
   - 「你現在有沒有想傷害自己的念頭?」
   - 「你有沒有想自殺的念頭?」
   - 「你有沒有具體計畫?」
   - 「你有想到怎麼做嗎?」
   Step 4 1925 給完後 → 下一輪走 Step 6 landing reminder. 不回頭重問.

❌ 沿用既有 safety-planning 護欄:
   - 不問「自殺方法」(會強化記憶).

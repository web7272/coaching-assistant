...
import {
  ...
  // ⭐ 6/7 P0 safety (Vivi A016): single-source prohibition block embedded in
  //   every Step 4 inject so AI rules persist across multi-turn SOP flow.
...
export function getHandoffInject(variant) {
  ...
  let body;
  switch (variant) {
    case 'standard':   body = HANDOFF_4_1_STANDARD; break;
    case 'cumulative':
    case 'remove_c':   // back-compat alias, deprecated
      body = HANDOFF_4_2_CUMULATIVE; break;
    case 'high_risk':
    case 'only_b':     // back-compat alias, deprecated
      body = HANDOFF_4_3_HIGH_RISK; break;
    case 'freeze':     body = HANDOFF_4_3B_FREEZE; break;
    default:
      throw new Error(`getHandoffInject: unknown variant ${variant}`);
  }
  // ⭐ 6/7 P0 safety — embed crisis output prohibitions (Vivi A016 production
  //   smoke). One source-of-truth block (_phrasings.js), one append site here.
  return `${header}${body}\n\n${CRISIS_OUTPUT_PROHIBITIONS}`;
}

...
test('🛑 6/7 P0: button text is the spec wording "登入 →"', () => {
  assert.match(script, /['"]登入 →['"]/);
});

test('🛑 6/7 P0: confirm headline is "準備好了嗎?點下面登入。" (per spec)', () => {
  assert.match(script, /準備好了嗎.?點下面登入/);
});
...
test('🛑 6/7 click handler: button text → "登入中…" while POST is in flight', () => {
  assert.match(script, /['"]登入中…['"]/);
});

...
// ─────────────────────────────────────────────────────────
// Regression / backward transitions（PR-4b 補、限 2 個合法 case）
//   - phase_3a → phase_3b: P10 regression
//       (Build Vision dissociated→associated 失敗、學員實際還 ambiguous)
//   - phase_3b → phase_3a: 3b→3a simplified
//       (Scope Overlap 升級 owned 後、進 Phase 3a Simplified)
// forward-only canTransitionPhase 保留、backward 走 canTransitionPhaseWithRegression
// ─────────────────────────────────────────────────────────

export const REGRESSION_TRANSITIONS = Object.freeze({
  phase_3a: Object.freeze([
    Object.freeze({ to: 'phase_3b', reason: 'p10_regression' }),
  ]),
  phase_3b: Object.freeze([
    Object.freeze({ to: 'phase_3a', reason: 'scope_overlap_to_simplified' }),
  ]),
});

export const REGRESSION_REASONS = Object.freeze([
  'p10_regression',
  'scope_overlap_to_simplified',
]);
...
/**
 * Can current_phase transition from → to, INCLUDING regression / backward?
 *
 * Allows the forward transitions (canTransitionPhase) PLUS the 2 legal
 * regression transitions in REGRESSION_TRANSITIONS, when `regressionReason`
 * matches. forward-only `canTransitionPhase` is unchanged —普通 advance 用它.
 *
 * @param {string} from
 * @param {string} to
 * @param {string} [regressionReason] - required for a regression transition
 * @returns {boolean}
 */
export function canTransitionPhaseWithRegression(from, to, regressionReason) {
  if (canTransitionPhase(from, to)) return true;
  if (!isValidCurrentPhase(from) || !isValidCurrentPhase(to)) return false;
  const regs = REGRESSION_TRANSITIONS[from];
  if (!Array.isArray(regs)) return false;
  return regs.some(r => r.to === to && r.reason === regressionReason);
}

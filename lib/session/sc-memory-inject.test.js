...
test('🛑 Vivi 6/19 B: scJourneyStep>=6 → 身分轉化 4 步指引 (英雄式歡迎+微證據+鑽石, 白話不用術語, 不強迫)', () => {
  ...
  assert.match(inj, /科學家模式/);
  assert.match(inj, /鑽石/);
  assert.match(inj, /①[\s\S]*②[\s\S]*③[\s\S]*④/);
...
test('🛑 Vivi 6/19 B: scJourneyStep<6 (或未傳) → 維持通用「往前推」指引, 不出 4 步', () => {
  const base = {
    valuesCollected: ['自由'], top1: '自由',
    scJourneyEvidence: { step_4: [{ type:'identity_claim', quote:'我是一心一意的人' }] },
    primaryMode: 'elicitation', flagEnabled: true,
  };
  const lo = buildScMemoryInject({ ...base, scJourneyStep: 5 });
  const none = buildScMemoryInject({ ...base });   // 未傳 step
  for (const inj of [lo, none]) {
    assert.match(inj, /從尚未走的步驟\(step_5 資源/);
    assert.equal(/英雄式歡迎/.test(inj), false);
  }
  assert.equal(CLIMAX_STEP_THRESHOLD, 6);

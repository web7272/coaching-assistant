...
// ═════════════════════════════════════════════════════════
// 🛑 fable 6/26 審查 P0 — aspirational identity_claim 不得當 owned 餵回 (A009 class)
// ═════════════════════════════════════════════════════════

test('🛑 fable P0: aspirational owned (想成為/希望我是) 從 step_4 濾掉、只留 confirmed', () => {
  const owned = extractOwnedIdentities({
    step_4: [
      { type: 'identity_claim', quote: '想成為全新的人' },      // aspirational → 濾
      { type: 'identity_claim', quote: '希望我是勇敢的人' },    // aspirational → 濾
      { type: 'identity_claim', quote: '也許我可以是溫柔的人' },// hedged → 濾
      { type: 'identity_claim', quote: '會撐住的人' },          // confirmed → 留
      { type: 'identity_claim', quote: '我是一心一意的人' },    // confirmed(我是) → 留
    ],
  });
  assert.deepEqual(owned, ['會撐住的人', '我是一心一意的人'],
    'aspirational / hedged 全濾、只留 confirmed present-tense');
});

test('🛑 fable P0: 全 aspirational → owned 空 → inject 無「已認領身份」段', () => {
  const inj = buildScMemoryInject({
    valuesCollected: ['自由'],
    scJourneyEvidence: { step_4: [{ type: 'identity_claim', quote: '想成為更好的自己' }] },
    flagEnabled: true,
  });
  assert.ok(inj, 'values 非空 → inject 仍出');
  assert.equal(/已認領身份/.test(inj), false,
    'aspirational 不得產生「已認領身份」段(A009 鬼打牆修正)');
});

...
/**
 * v5.0 spec 09 §12 — phase 2–5 跨日 banner. 對話 phase 改天數驅動後 phase 2–5
 * 首次真的會跑到 (之前永遠卡 phase_1); 它們是 static string、沒像 phase_1 那樣
 * 有 day_opening_inject 變體。跨階邊界 (e.g. Day 5 進 phase_2) 若不處理會冷啟、
 * 破壞跨日連續性。banner 前置在 base text 前讓 Sonnet 先依 E4 inject 接住昨天、
 * 再帶進今天 phase 的主題。
 */
const CROSS_DAY_DEFER_BANNER = '🛑 本 turn 是「跨日開場」：先依 [SYSTEM INJECT — Day Opening Active Reference] 溫暖接住昨天的素材（不直引、不評估），再自然帶進今天本階段的主題。不要冷啟、不要忽略昨天。';
 ...
 *
 * PR-4c-green E4 fix — opts.dayOpeningInjectActive:
 *   - phase_1 + opening: swap to day_opening_inject variant (already has 跨日 language)
 *   - phase 2–5 (static string): prepend CROSS_DAY_DEFER_BANNER (spec 09 §12, 5/25)
 *
...
export function contextFor(currentPhase, routerPhase, opts = {}) {
  ...
  if (typeof entry === 'string') {
    ...
    // phase_1 跨日：用既有 day_opening_inject 變體（已含跨日語言、自帶禁冷起手式）、直接回
    if (dayOpen
  ...
  // v5.0 spec 09 §12 — phase 2–5 (string entry) 跨日開場時前置 banner、避免跨階冷啟.
  // phase_1 的跨日路徑已在上面 day_opening_inject 變體處理掉、不會走到這裡.
  if (dayOpen && currentPhase !== 'phase_1' && base) {

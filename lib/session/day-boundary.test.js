...
// ─────────────────────────────────────────────────────────
// calcCalendarDayDiff (Asia/Taipei midnight math, 5/27 Patrick)
//   原本 UTC midnight、台北凌晨 1 點 (UTC 17:00 of「prev」 day) 仍算「昨天」 →
//   daily 學員的 Day 2 永遠不解鎖. 改後以 +UTC 8 取台北日為界.
// ─────────────────────────────────────────────────────────

test('calcCalendarDayDiff: same Taipei day → 0 (5/20 02:00 → 22:00 台北)', () => {
  // Taipei 5/20 02:00 = UTC 5/19 18:00 / Taipei 5/20 22:00 = UTC 5/20 14:00.
  const a = new Date(Date.UTC(2026, 4, 19, 18, 0, 0));
  const b = new Date(Date.UTC(2026, 4, 20, 14, 0, 0));
  assert.equal(calcCalendarDayDiff(a, b), 0);
});

test('calcCalendarDayDiff: 1 Taipei day later → 1 (5/20 22:00 → 5/21 09:00 台北)', () => {
  const a = new Date(Date.UTC(2026, 4, 20, 14, 0, 0));   // Taipei 5/20 22:00
  const b = new Date(Date.UTC(2026, 4, 21,  1, 0, 0));   // Taipei 5/21 09:00
...
// 🛑 5/27 Patrick (封測 bug 根因 ①) — Taipei boundary 關鍵 3 條.
//   證明改成台北日界、不再用 UTC.

test('🛑 calcCalendarDayDiff: same Taipei day, late UTC morning → 0', () => {
  // Taipei 5/27 10:00 (UTC 5/27 02:00) → Taipei 5/27 21:00 (UTC 5/27 13:00).
  const from = new Date(Date.UTC(2026, 4, 27, 2, 0, 0));
  const to   = new Date(Date.UTC(2026, 4, 27, 13, 0, 0));
  assert.equal(calcCalendarDayDiff(from, to), 0);
});

test('🛑 calcCalendarDayDiff: crosses Taipei midnight via UTC afternoon → 1', () => {
  // Taipei 5/27 10:00 (UTC 5/27 02:00) → Taipei 5/28 01:00 (UTC 5/27 17:00).
  const from = new Date(Date.UTC(2026, 4, 27,  2, 0, 0));
  const to   = new Date(Date.UTC(2026, 4, 27, 17, 0, 0));
  assert.equal(calcCalendarDayDiff(from, to), 1);
});

test('🛑 calcCalendarDayDiff: same Taipei day but CROSSES UTC midnight → 0 (was wrong before)', () => {
  // Taipei 5/28 06:00 (UTC 5/27 22:00) → Taipei 5/28 10:00 (UTC 5/28 02:00).
  // Pre-5/27 (UTC 日界): would return 1 (跨 UTC day). After: returns 0.
  // This is THE smoking-gun test that proves the boundary moved to Taipei.
  const from = new Date(Date.UTC(2026, 4, 27, 22, 0, 0));
  const to   = new Date(Date.UTC(2026, 4, 28,  2, 0, 0));
  assert.equal(calcCalendarDayDiff(from, to), 0,
    '同台北日 (5/28) 但跨 UTC midnight 應該回 0、證明用台北日界');
});

// ─────────────────────────────────────────────────────────
// taipeiDateString (5/27 Patrick)
// ─────────────────────────────────────────────────────────

test('taipeiDateString: UTC 17:00 → Taipei next day (5/27 17:00Z = 5/28 01:00 Taipei)', () => {
  assert.equal(taipeiDateString(new Date('2026-05-27T17:00:00Z')), '2026-05-28');
});

test('taipeiDateString: UTC 02:00 → same Taipei day (5/27 02:00Z = 5/27 10:00 Taipei)', () => {
  assert.equal(taipeiDateString(new Date('2026-05-27T02:00:00Z')), '2026-05-27');
});

test('taipeiDateString: UTC 22:00 → Taipei next day (5/27 22:00Z = 5/28 06:00 Taipei)', () => {
  assert.equal(taipeiDateString(new Date('2026-05-27T22:00:00Z')), '2026-05-28');
});

test('taipeiDateString: month rollover (5/31 22:00Z = 6/01 06:00 Taipei)', () => {
  assert.equal(taipeiDateString(new Date('2026-05-31T22:00:00Z')), '2026-06-01');
});

test('taipeiDateString: rejects non-Date', () => {
  assert.throws(() => taipeiDateString('2026-05-27'), TypeError);
  assert.throws(() => taipeiDateString(null), TypeError);
});
...
test('detectNewSessionDay: same Taipei day → is_new_day:false', () => {
  // Taipei 5/20 17:00 → Taipei 5/20 22:00 (both Taipei 5/20).
  const last = new Date(Date.UTC(2026, 4, 20,  9, 0, 0));   // 17:00 Taipei
  const now  = new Date(Date.UTC(2026, 4, 20, 14, 0, 0));   // 22:00 Taipei (same day)
  const r = detectNewSessionDay({ last_active_date: last }, now);
  assert.equal(r.is_new_day, false);
  assert.equal(r.gap_days, 0);
});

test('detectNewSessionDay: next Taipei day → is_new_day:true, gap_days:1', () => {
  // Taipei 5/20 22:00 → Taipei 5/21 09:00.
  const last = new Date(Date.UTC(2026, 4, 20, 14, 0, 0));   // 22:00 Taipei (5/20)
  const now  = new Date(Date.UTC(2026, 4, 21,  1, 0, 0));   // 09:00 Taipei (5/21)
  const r = detectNewSessionDay({ last_active_date: last }, now);

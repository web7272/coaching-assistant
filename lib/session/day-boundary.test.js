...
// 守則 2 最關鍵 regression：reset patch 絕對不含 phase progress 6 欄位。
...
// ─────────────────────────────────────────────────────────
// ⭐ regression：phase progress 6 欄位絕不在 reset patch
// ─────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────
// reset patch 完整性（24 欄位、按 migration 014 §3）
// ─────────────────────────────────────────────────────────

test('RESET_FIELDS contains exactly 23 transient fields per migration 014 §3', () => {
  const expected = [
    // Engine 1 (8)
    ...
    // Checkpoint 1 mid-session counter (1)
...
test('buildResetPatch returns a fresh object each call (no aliasing const)', () => {
  ...
  assert.deepEqual(b.deep_signal_flags, {}, 'nested objects must be deep-copied');
});

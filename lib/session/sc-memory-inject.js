...
// 🔴 fable 6/26 審查 P0 — owned ≠ aspirational.
//   observer 把 aspirational identity_claim(「想成為 X」/「希望我是 X」)也記進
//   step_4(sc-observer §6, A009 case),但那不是「已認領」。餵回若當 owned →
//   Sonnet 會跳過本該做的認領工作。讀取點濾掉 aspirational / hedged,只留 confirmed
//   present-tense。對齊 observer「better empty than messy」:寧可少收、不可誤收。
export const ASPIRATIONAL_OWNED_RE =
  /想成為|想變成|想當|想做個|想做一個|想要成為|想要變成|渴望成為|希望我是|希望自己是|希望成為|如果我(?:是|能)|要是我是|但願|也許我(?:是|可以)|我可以是|想擁有|想變得/;
export const HEDGED_OWNED_RE =
  /好像我是|感覺我(?:好像)?是|應該算是|大概是|可能是|說不定我/;
...
export function extractOwnedIdentities(scJourneyEvidence) {
  ...
  for (const e of step4) {
    ...
    if (ASPIRATIONAL_OWNED_RE.test(q) || HEDGED_OWNED_RE.test(q)) continue;   // fable P0: 濾 aspirational/hedged,不當 owned

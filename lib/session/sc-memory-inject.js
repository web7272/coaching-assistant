...
export const STEP6_IDENTITY_SHIFT_GUIDANCE = [
  ...
  '⚠️ 不強迫改變(強迫只會增加內在衝突)、不要再重複「你把價值交給別人」這種已經講過很多次的點破。改走這 4 步,讓「想改」從她自己長出來:',
  '① 揭露代價:用反問讓她自己看見——「是誰規定『沒被稱讚=沒用』的?」把『我好不好』交給別人,像追一根追不到的紅蘿蔔,永遠焦慮;而且一旦換到不給你掌聲的地方,你的自信會立刻垮。',
  ...
  '最終立場:她若仍選擇不改,尊重她(不選擇也是一種選擇),但提醒她要為「把決定權交給別人」的代價(持續的焦慮與不穩)負責。真正的轉變,發生在她看見:她手裡那顆鑽石,不需要鑑定師,也會自己發光。',
].join('\n');

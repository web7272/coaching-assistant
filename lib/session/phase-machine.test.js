...
import {
  ...
  isValidRouterPhase,
  canTransitionPhase,
  canTransitionPhaseWithRegression,
  canSetRouterPhase,
  advancePhase,
  setRouterPhase,
...
// ─────────────────────────────────────────────────────────
// canTransitionPhaseWithRegression (PR-4b — 2 legal backward cases)
// ─────────────────────────────────────────────────────────

test('REGRESSION_TRANSITIONS / REGRESSION_REASONS are frozen', () => {
  assert.ok(Object.isFrozen(REGRESSION_TRANSITIONS));
  assert.ok(Object.isFrozen(REGRESSION_REASONS));
});

test('REGRESSION_REASONS: exactly 2 legal reasons', () => {
  assert.deepEqual(
    [...REGRESSION_REASONS].sort(),
    ['p10_regression', 'scope_overlap_to_simplified'],
  );
});

test('canTransitionPhaseWithRegression: still allows all forward transitions', () => {
  assert.equal(canTransitionPhaseWithRegression('phase_1', 'phase_2'), true);
  assert.equal(canTransitionPhaseWithRegression('phase_2', 'phase_3a'), true);
  assert.equal(canTransitionPhaseWithRegression('phase_3a', 'phase_4'), true);
  assert.equal(canTransitionPhaseWithRegression('phase_5', 'program_completed'), true);
});

test('canTransitionPhaseWithRegression: phase_3a → phase_3b with p10_regression', () => {
  assert.equal(
    canTransitionPhaseWithRegression('phase_3a', 'phase_3b', 'p10_regression'),
    true,
  );
});

test('canTransitionPhaseWithRegression: phase_3b → phase_3a with scope_overlap_to_simplified', () => {
  assert.equal(
    canTransitionPhaseWithRegression('phase_3b', 'phase_3a', 'scope_overlap_to_simplified'),
    true,
  );
});

test('🛑 canTransitionPhaseWithRegression: backward needs the CORRECT reason', () => {
  // right edge, wrong reason → blocked
  assert.equal(
    canTransitionPhaseWithRegression('phase_3a', 'phase_3b', 'scope_overlap_to_simplified'),
    false,
  );
  assert.equal(
    canTransitionPhaseWithRegression('phase_3b', 'phase_3a', 'p10_regression'),
    false,
  );
  // no reason at all → blocked
  assert.equal(canTransitionPhaseWithRegression('phase_3a', 'phase_3b'), false);
  assert.equal(canTransitionPhaseWithRegression('phase_3b', 'phase_3a'), false);
});

test('🛑 canTransitionPhaseWithRegression: does NOT open arbitrary backward jumps', () => {
  // phase_4 → phase_2 is not a registered regression
  assert.equal(canTransitionPhaseWithRegression('phase_4', 'phase_2', 'p10_regression'), false);
  assert.equal(canTransitionPhaseWithRegression('phase_5', 'phase_1', 'scope_overlap_to_simplified'), false);
  // even a real reason can't unlock an unregistered edge
  assert.equal(canTransitionPhaseWithRegression('phase_3b', 'phase_2', 'p10_regression'), false);
});

test('canTransitionPhaseWithRegression: rejects unknown phases', () => {
  assert.equal(canTransitionPhaseWithRegression('phase_99', 'phase_3b', 'p10_regression'), false);
  assert.equal(canTransitionPhaseWithRegression('phase_3a', 'phase_99', 'p10_regression'), false);
});

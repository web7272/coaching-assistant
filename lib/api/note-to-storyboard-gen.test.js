...
test('🛑 Vivi 6/19 C: 放寬內控宣告字樣 (慣用語) + 窗口 30→50', () => {
  // 新字樣都認得 (gateSovereign 收尾)
  for (const tail of [
    '把那張票還回去。我自己按讚存檔。',
    '她的選擇是她的，你的價值由我決定。',
    '我的價值由我，不靠那張票。',
    ...
    '裁判權在我。',
    '老子自己說了算。',
  ]) assert.equal(gateSovereign(tail).ok, true, tail);
  // 窗口 30→50:宣告在收尾區 (距尾 >30 但 <50) 仍算數
  const buried = '那是她的處境不是你的評分，由我決定我是誰，' + '剩下的慢慢來都可以。'; // 宣告後還拖一句
  assert.equal(endsWithSelfControl(buried), true, '宣告在最後 50 字內 → 認得');
  // 反向:真的沒宣告仍 false (不放水)
  assert.equal(endsWithSelfControl('你本身值得被完整選擇。'), false, '純肯定句、無內控宣告 → 仍 false');
  // 指向外的「她說了算」不可誤判為內控
  assert.equal(endsWithSelfControl('到頭來還是她說了算。'), false, '她說了算 ≠ 內控宣告');
});
...
// ── buildScStoryboardFromGen ──
test('🛑 buildScStoryboardFromGen: gen 內容決定 filled (不再被 observer cap 砍) + new fields + round-trip', () => {
  const gen = {
  ...
  // Vivi 6/19 A: step 7 有 gen 內容 → filled, 不再被 currentStep(6) 砍掉
  assert.equal(sb.step_7.state, 'filled');
  assert.equal(sb.step_7.brain_state.description, '大腦7');
  assert.equal(sb.step_7.sovereign_action, '宣告7。我說了算。');
...
test('🛑 S6 regenStoryboardFromNotes: writes new-shape sc_storyboard (filled>0), fills per gen content', async () => {
  const d = regenDeps();
  ...
  assert.equal(sb.step_7.state, 'empty'); // SYNTH_FULL 無 step_7 內容 → empty (非 cap)
});
...
// ── 只升不降 merge (Vivi 6/19 D) ──
test('🛑 D mergeStoryboardKeepBest: 不降級 (空/缺宣告不覆寫已 filled/已宣告);complete 才 refresh', () => {
  const filled = (sov) => ({ state:'filled', brain_state:{description:'x',quote:null}, resistance:null, benevolent_intent:null, sovereign_action: sov ?? null });
  const empty = { state:'empty', brain_state:null, resistance:null, benevolent_intent:null, sovereign_action:null };
  const oldSb = { step_4: filled(), step_6: filled('宣告。我說了算。'), step_7: filled('宣告。我說了算。') };
  const neu = {
    step_4: empty,                         // 新空 → 保留舊 filled
    step_6: filled(null),                  // 新缺宣告 (partial) → 保留舊有宣告
    step_7: filled('新宣告。主導權在我。'),  // 新 complete → refresh 用新
  };
  const m = mergeStoryboardKeepBest(oldSb, neu);
  assert.equal(m.step_4.state, 'filled', '不降級到空');
  assert.equal(m.step_6.sovereign_action, '宣告。我說了算。', '缺宣告不覆寫舊宣告');
  assert.equal(m.step_7.sovereign_action, '新宣告。主導權在我。', 'complete→complete refresh');
  // 升級:新 filled + 舊空 → 用新
  const m2 = mergeStoryboardKeepBest({ step_1: empty }, { step_1: filled() });
  assert.equal(m2.step_1.state, 'filled');
});

test('🛑 D regenStoryboardFromNotes: readStoryboard 接線 → 合併保留舊 filled 步 (不被新版洗空)', async () => {
  const d = regenDeps();
  const oldSb = { step_7: { state:'filled', brain_state:{description:'舊步7',quote:null}, resistance:null, benevolent_intent:null, sovereign_action:'舊宣告。我說了算。' } };
  d.readStoryboard = async () => oldSb;
  const r = await regenStoryboardFromNotes({ studentId:'A006', currentStep:7, ...d, model:'x' });
  assert.equal(r.wrote, true);
  const sb = d.writes[0].sb;
  // SYNTH_FULL 沒 step_7 → 新版 step_7 空,但舊有 filled → 合併後保留
  assert.equal(sb.step_7.state, 'filled');
  assert.equal(sb.step_7.sovereign_action, '舊宣告。我說了算。');
});

...
/**
 * Canonical v5 CP1 phase ordering. Index in this array = monotonic "phase
 * advancement" measure for the Phase Report unlock thresholds. Returns -1
 * for unknown / null current_phase (= brand-new student, all reports locked).
 *
 * @param {string|null|undefined} phase
 * @returns {number} -1..7
 */
export function phaseSequenceIndex(phase) {
  const seq = [
...
/**
 * Phase Report unlock thresholds — Report N unlocks when phaseSequenceIndex >
 * the value at index N-1. Tuned so that Phase Report 3「擴大地圖」 unlocks
 * after EITHER the 3a (owned) OR the 3b (ambiguous) path, since the student
 * walks only one of them.
 *
 *   Report 1 unlocks past phase_1            (cpIdx > 0)
 *   Report 2 unlocks past phase_2            (cpIdx > 1)
 *   Report 3 unlocks past phase_3b           (cpIdx > 3)  — 3a path: cpIdx=4 at phase_4 ✓
 *   Report 4 unlocks past phase_4            (cpIdx > 4)
 *   Report 5 unlocks past phase_5            (cpIdx > 5)
 */
const PHASE_REPORT_UNLOCK_THRESHOLDS = [0, 1, 3, 4, 5];

/**
 * Compute the 5 Phase Report states given the student's current v5 phase.
 *
 * @param {string|null|undefined} currentPhase  e.g. 'phase_1', 'phase_3b', null
 * @returns {Array<{phaseId:number, name:string, state:'locked'|'unlocked'}>}  length 5
 */
export function computePhaseReportStates(currentPhase) {
  const cpIdx = phaseSequenceIndex(currentPhase);
 ...
 * current_phase? Used by /api/phase-report to gate the read.
 *
 * @param {number} phaseId  1..5
 * @param {string|null|undefined} currentPhase
 * @returns {boolean}
 */
export function isPhaseReportUnlocked(phaseId, currentPhase) {
  if (!Number.isInteger(phaseId) || phaseId < 1 || phaseId > 5) return false;
  return phaseSequenceIndex(currentPhase) > PHASE_REPORT_UNLOCK_THRESHOLDS[phaseId - 1];
}

...
test('🛑 computeUnlockedCurrentDay: daily mode → no auto-advance on completion', () => {
  // Day 1 just completed in daily mode → still on Day 1 until calendar day rolls over
  assert.equal(
    computeUnlockedCurrentDay({ pace: 'daily', sessionDayCount: 1, lastSessionComplete: true }),
    1,
  );
  assert.equal(
    computeUnlockedCurrentDay({ pace: 'daily', sessionDayCount: 5, lastSessionComplete: true }),
    5,
  );
});

...
export const SOVEREIGN_ACTION_PROTOTYPES = Object.freeze({
  ...
  6: '撕開「對方的選擇」 vs「你的價值」 → 收回裁判權 → 愛轉回自己。',
 ...
 *     * LLM throws / returns no text
 *     * output rigidity-at-self detected
 *     * output missing self-control declaration
 *     * scrubber strips to empty
 */
export async function generateSovereignAction({
  ...
  try {
    ...
    if (!result?.ok || !result.data?.content?.[0]?.text) {
      log(`[sc-storyboard-gen][sovereign_action] step ${stepNo} no LLM text → null`);
      return null;
    }
    raw = result.data.content[0].text.trim();
  } catch (err) {
    log(`[sc-storyboard-gen][sovereign_action] step ${stepNo} LLM threw: ${err?.message || err}`);
    return null;
  }
  ...
  if (!cleaned) {
    log(`[sc-storyboard-gen][sovereign_action] step ${stepNo} scrubbed to empty → null`);
    return null;
  }
  // 🔴 ship-gate: rigidity must NOT point at self.
  if (_pointsRigidityAtSelf(cleaned)) {
    log(`[sc-storyboard-gen][sovereign_action] step ${stepNo} rigidity-at-self detected → null`);
    return null;
  }

...
export function endsWithSelfControl(t) {
  ...
  return SELF_CONTROL_DECLARATION_PATTERNS.some(p => p.test(t.slice(-50)));  // Vivi 6/19 C: 30→50 (宣告在收尾區即可)
}
...
export function buildSovereignRetrySystem(stepNo) {
  return [
    ...
    '⚠️ 最後一句**必須**是一句明確的內控宣告,逐字收在下列其中一種句式上(不要收在肯定句或描述句):',
    '   「…我自己說了算。」「…主導權在我。」「…由我決定。」「…我自己按讚存檔。」「…不需要任何人點頭/認可。」',
...
export function buildScStoryboardFromGen(genSteps, currentStep) {
  // Vivi 6/19 (A): note 生成的內容自己決定顯示哪幾步 — 有 brain_state 的步就 filled,
  //   不再被 observer 的 currentStep (sc_journey_step) 往下砍。synthesis 對材料不足的
  //   步本來就回 '' 留白 → 自我節制,不會替早期學員亂開高步。currentStep 參數保留
  //   (相容呼叫端) 但不再當 cap。
  const sb = {};
  for (let n = 1; n <= 7; n++) {
...
// ── S6: finalize 接線用 best-effort 重生 helper ───────────────────
// 從學員全部 damon_notes 重生 sc_storyboard (新結構) 並寫回。
//   best-effort:任何失敗 → 回 {wrote:false} 並保留既有 storyboard, 絕不 throw。
//   gen 產 0 filled → 不寫 (避免把既有內容洗成空)。
//   deps 注入 (可測):pullNotes(id)->rows · writeStoryboard(id, sb) · anthropic · callAnthropicWithRetry。
// ── 只升不降 merge (Vivi 6/19 D) ──────────────────────────────
// 每天 finalize 全量重生時, 不讓「已 filled / 已有主權宣告」的步被空版或缺宣告版覆寫。
// 品質分級 (per step):
//   2 = complete (步1-5: filled;步6-7: filled 且有 sovereign_action)
//   1 = partial  (步6-7: filled 但 sovereign null)
//   0 = empty
// 規則:新 >= 舊 → 用新 (refresh / upgrade);舊嚴格較高 → 保留舊 (不降級)。
export function _scStepQuality(key, step) {
  if (!step || step.state !== 'filled') return 0;
  const n = Number(String(key).replace('step_', ''));
  if ((n === 6 || n === 7) && !step.sovereign_action) return 1;
  return 2;
}
export function mergeStoryboardKeepBest(oldSb, newSb) {
  const out = {};
  for (let n = 1; n <= 7; n++) {
    const k = `step_${n}`;
    const nv = newSb && newSb[k];
    const ov = oldSb && oldSb[k];
    out[k] = (_scStepQuality(k, ov) > _scStepQuality(k, nv)) ? ov : nv;
  }
  return out;
}

export async function regenStoryboardFromNotes({
  studentId,
  currentStep,
  pullNotes,
  writeStoryboard,
  ...
  try {
    ...
    const gen = await generateStoryboardFromNotes({
      notes, student: studentId, anthropic,
      callAnthropicWithRetry: callAnthropic, model, log,
    });
    let sb = buildScStoryboardFromGen(gen.steps, currentStep);
    // 只升不降 (D):讀舊的、逐步保留品質較高版本 → 不讓 filled/有宣告的步被空版洗掉.
    if (typeof readStoryboard === 'function') {
      try {
        const oldSb = await readStoryboard(studentId);
        if (oldSb && typeof oldSb === 'object') sb = mergeStoryboardKeepBest(oldSb, sb);
      } catch (e) { log(`[regenStoryboardFromNotes] readStoryboard failed (用新版): ${e?.message || e}`); }
    }
    const filled = Object.keys(sb).filter(k => sb[k] && sb[k].state === 'filled').length;
    if (filled <= 0) { log('[regenStoryboardFromNotes] 0 filled → keep existing'); return { wrote: false, filled: 0 }; }
    await writeStoryboard(studentId, sb);
    return { wrote: true, filled, incongruence_detected: !!gen.incongruence_probe?.detected };
  } catch (e) {
    log(`[regenStoryboardFromNotes] best-effort failed (kept existing): ${e?.message || e}`);
    return { wrote: false, error: e?.message || String(e) };
  }
}

...
export const CASCADE_PRIORITY = Object.freeze({
  // ⭐ Step 6 PR-6a — Crisis SOP state machine fires FIRST when crisis_in_progress.
  //   Only fires when state.crisis_in_progress OR primary_mode == 'crisis'.
  //   When handled=true, cascade short-circuits, all other sub-routers skip.
  CRISIS_sop:                    2,  // crisis-sop.js — 9-step SOP state machine
  E1_deviation_pipeline:        10,  // E1 master + classifier + E1a/b/c/d（最優先）
  E3_deep_signal_detector:      20,  // 深創傷訊號（不能被 elicitation 吞）
  E3_passive_hope_cascade:      25,  // passive hope cascade (PR-23s4c task 4, after deep_signal)
  ...
  // PR-23s4c — task 2 new sub-routers:
  E3_integration_router:        45,  // integration mode toolbox (PR-23s4c task 2 新增)
  ...
  E3_future_pacing_router:      65,  // future_pacing mode dispatcher (PR-23s4c task 2 新增)

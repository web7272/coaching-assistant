...
test('🔴 6/14 A006 漏網修: 「傷害自己」family → skip="high_risk"', () => {
  // Real leaked quote from A006 step_6 brain_state (dryRun 漏網):
  //   「你不用擔心我會傷害我自己，縱使我是一個人待著」
  // — 雖康復語氣, 但寫死「傷害我自己」, 既有 /自傷/ 沒擋到. 此測鎖修正後行為.
  for (const userText of [
    ...
    const r = shouldSkipPreLLM({
      turn: { user: userText, ai: '想聽你說' },
      context: { primary_mode: 'elicitation' },
    });
    assert.equal(r?.skip_reason, 'high_risk',
      `「${userText}」必須觸發 high_risk skip (6/14 denylist 補強後)`);
  }
});
...
test('🔴 SYSTEM_PROMPT: 6/12 step_7 anchor capture explicit', () => {
  ...
  assert.match(SYSTEM_PROMPT, /A006[\s\S]{0,80}照顧/,
    'Prompt must cite A006 「照顧」 case as the exemplar');
});

test('🔴 SYSTEM_PROMPT: 6/12 round 2 — step_7 strengthen (3 exemplars + learner-side + 今天先到這裡)', () => {
  // Round 2: real eval found 0 captures. Lowered bar + more patterns + 3 exemplars.
  assert.match(SYSTEM_PROMPT, /round 2|STRENGTHEN/i,
    'Prompt must mark round-2 step_7 strengthening');
  assert.match(SYSTEM_PROMPT, /今天先到這裡/,
    'Round 2 must add 「今天先到這裡」 session-close + anchor pattern');
  // Learner-side: 對, 我是 pattern.
  assert.match(SYSTEM_PROMPT, /對, 我是/,
    'Round 2 must include 「對, 我是」 learner deliberate acceptance pattern');
  // 3 exemplars all present: A006 照顧, A003 剛剛好的自由, A005 自然真誠.
  assert.match(SYSTEM_PROMPT, /A003[\s\S]{0,120}剛剛好的自由/,
    'Round 2 must cite A003 「剛剛好的自由」 as exemplar');
  assert.match(SYSTEM_PROMPT, /A005[\s\S]{0,120}自然真誠/,
    'Round 2 must cite A005 「自然真誠」 as exemplar');
  assert.match(SYSTEM_PROMPT, /ERR ON THE SIDE OF CAPTURING|over-capture/,
    'Round 2 must instruct to lean toward capturing (vs missing)');
});
...
test('🔴 SYSTEM_PROMPT: 6/12 round 3 — aspirational ≠ owned (A009 case fix)', () => {
  // Round 3: observer was capturing 「想成為全新的人」 as owned (wrong).
  // Aspirational claims belong to step_4 identity_claim, NOT owned.
  assert.match(SYSTEM_PROMPT, /round 3/i,
    'Prompt must mark round-3 aspirational rule');
  assert.match(SYSTEM_PROMPT, /ASPIRATIONAL/i);
  assert.match(SYSTEM_PROMPT, /想成為|想變成/,
    'Prompt must explicitly list aspirational patterns');
  assert.match(SYSTEM_PROMPT, /step_4 identity_claim/,
    'Prompt must direct aspirational claims to step_4 identity_claim');
  // A009 specific exemplar.
  assert.match(SYSTEM_PROMPT, /A009[\s\S]{0,200}成就感/,
    'Round 3 must cite A009 「成就感」 as the correct owned (vs 「全新的人」 wrong)');
});

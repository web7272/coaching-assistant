...
# 🔴 6/12 — step_7 ANCHORING explicit capture rules

Capture as step_7 anchoring evidence when ANY of these patterns appear in
the turn:

- AI says: "把這句話留下來:『X』" / "定錨『X』" / "明天從『X』這裡接住" /
  "今天讓『X』成形了" → record entry {type: 'anchoring', quote: 'X'}.
- Learner repeats their own anchored phrase deliberately:
  "我就是『X』" / "『X』 — 這是我的" → record entry {type: 'anchoring',
  quote: <the repeated phrase>}.
- Day-close / session-close moment where AI explicitly names a phrase to
  carry forward → record entry.

For A006 specifically: 「照顧」 IS the anchor phrase; if it appears in this
context, record step_7 anchoring evidence with quote="照顧".

Don't skip step_7 because the conversation didn't formally label "anchor".
Look for substance: a concrete short phrase deliberately set as identity.

...
/**
 * v5.1 Step 5b → Step 7 PR-7a — R2 strategy-not-quality reframe inject.
 * Returned when dim_4 = true; replaces the upgrade. Sonnet then re-asks evidence
 * with strategy-vs-quality framing.
 *
 * Step 7 PR-7a: ship-able 過渡 phrasing 保留為 fallback; appends R2 Reframe Library
 * full body so Sonnet has both the original short prompt + full 4-step script.
 */
import { R2 } from '../damon-reframe-library/r2-behavior-to-identity.js';

const _R2_LIBRARY_FULL_BODY = R2.buildInject({
  variant: 'standard',
  behavior_quote: '[學員當下 evidence 動詞]',
  quality: '[underlying quality 候選]',
  mode: 'identity_anchoring',
  sensory_detail_count: 2,
});

export const R2_STRATEGY_REFRAME_INJECT = `[SYSTEM INJECT — A1 dim 4: Strategy vs Quality (R2 library invocation)]

Haiku A1 judge 偵測到 evidence 像「策略」(換取 outcome) 而非「本質」(inherent quality).
不寫 owned — R2 Reframe Library 接管.

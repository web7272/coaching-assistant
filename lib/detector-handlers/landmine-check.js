...
export function buildTier1RejectInject(termMatched) {
  ...
  // ⭐ Step 7 PR-7b — 自我重要性 Landmine 專屬深挖句 (errata v0.2 Patch 5.4 逐字).
  const selfImportanceTerms = ['證明自己', '證明自己的', '證明我是', '證明我是的',
    '讓人知道我是', '讓人知道我是的', '自我重要性', '自我重要性的',
    '自豪', '自豪的', '被看見', '被看見的', '被注意', '被注意的'];
  const isSelfImportance = selfImportanceTerms.includes(term);
  const selfImportanceDeepDig = isSelfImportance ? `

⭐ 自我重要性 Landmine 專屬深挖 (errata v0.2 Patch 5.4):
> 「我聽到了——『${term}』。
>  我想問你——
>
>  如果『${term}』這個 part 去掉、
>  你剩下什麼?
>
>  不是『我${term.startsWith('證明') ? '證明了什麼' : '想被看見什麼'}』、是『我是誰』。」

→ 引導 surface underlying genuine quality (通常是能力 / 授權 / 真實 / 連結).
→ 鏈式深挖句: 「『${term}』這個感覺、最深的、是想被什麼?」
` : '';
...
- 學員若堅持「${term}」是 Top 1 (M_LM_1): 不收 owned, 標 ambiguous + handoff.${selfImportanceDeepDig}
---

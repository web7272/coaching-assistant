...
import * as s6 from './s6-modal-operators.js';   // ⭐ Step 7 PR-7b — Modal Operators 應該/必須.
...
function addThresholdFlags(signal, newSessCount, newCumulative, patch) {
  ...
  if (signal === 'modal_operator') {
    if (newCumulative >= T.cumulative_e3_external_locus_pattern) {
      patch.s6_e3_external_locus_pattern_flag = true;
    }
  }
}
...
/**
 * S3 light path inject — frequency_illusion, R7 Reframe Library 接管 (Step 7 PR-7a).
 * 對齊 §3 紅線 22 既有 pattern「『X』是身份、不是成績單」+ R7_A 5-step full version.
 */
import { R7 } from '../../damon-reframe-library/r7-slip-into-unconscious.js';

const _R7A_FULL_BODY = R7.buildInject({
  variant: 'standard',
  frequency_check_quote: '[學員 surface 的成績單 / 頻率原話]',
  quality: '[quality 詞]',
  role: '[role / persona]',
  mode: 'identity_anchoring',
});

export const S3_LIGHT_INJECT = `[SYSTEM INJECT — Frequency Illusion (R7 library invocation)]
偵測到 frequency_illusion 訊號 (成績單 / 及格分數 / 頻率比較 / self-check).
R7 Reframe Library 接管完整 reframe.

...
test('🛑 updateCarryForwardOnSessionClose: 3 sessions no re-trigger → auto-resolved (PR-6b: caller passes invokedAt)', () => {
  ...
  }, { invokedAt: '2026-06-01T00:00:00Z' });
  cf = updateCarryForwardOnSessionClose(cf, false, { invokedAt: '2026-06-02T00:00:00Z' });  // session 1 no re-trigger
  assert.equal(cf.sessions_since_trigger, 1);
  assert.equal(cf.resolved_at, null);
  cf = updateCarryForwardOnSessionClose(cf, false, { invokedAt: '2026-06-03T00:00:00Z' });  // session 2
  assert.equal(cf.sessions_since_trigger, 2);
  assert.equal(cf.resolved_at, null);
  cf = updateCarryForwardOnSessionClose(cf, false, { invokedAt: '2026-06-04T00:00:00Z' });  // session 3 → auto-resolve
  assert.equal(cf.sessions_since_trigger, 3);
  assert.equal(cf.resolved_at, '2026-06-04T00:00:00Z',
    'resolved_at populated w/ caller-supplied ISO (no Date.now)');
...
test('Patch 1 cached: token_estimate bumped to 1570 (Step 7 PR-7b R8/R12 +120)', async () => {
  const { default: section } = await import('../prompt-sections/cached/active-reference-styles.js');
  assert.equal(section.token_estimate, 1570);
});

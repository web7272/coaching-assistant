...
test('🛑 errata v02: getHandoffInject(standard) — direct 1925, NO 三選一 a/b/c, NO 預約 Vivi', () => {
  const inject = getHandoffInject('standard');
  // Verbatim Step 4.1 errata v02:
  assert.match(inject, /我聽到了。/);
  assert.match(inject, /這句話很重——我想停在這裡、不繼續往前推。/);
  assert.match(inject, /如果你現在有立即的危險、請撥打 1925\(安心專線、24 小時\)。/);
...
test('🛑 errata v02: getHandoffInject(cumulative) — errata v02 step 4.2 + 諮商師 long-term framing', () => {
  const inject = getHandoffInject('cumulative');
  assert.match(inject, /過去幾次、你提過類似的。/);
  assert.match(inject, /我想停在這裡、不繼續往前推。/);
...
test('🛑 errata v02: getHandoffInject(remove_c) back-compat alias → cumulative', () => {
  ...
  // must still resolve to a phrasing (cumulative).
  const inject = getHandoffInject('remove_c');
  assert.match(inject, /過去幾次、你提過類似的。/);
});

test('🛑 errata v02: getHandoffInject(high_risk) — direct 1925 + 真人在你旁邊 + 不需要『真的要做什麼』', () => {
  const inject = getHandoffInject('high_risk');
  assert.match(inject, /你現在最需要的、是一個真人在你旁邊。/);
  assert.match(inject, /如果你現在有立即的危險、請撥打 1925\(安心專線、24 小時\)。/);
  assert.match(inject, /你不需要『真的要做什麼』才能打。只是很重、打過去就可以。/);
...
test('🛑 errata v02: getHandoffInject(only_b) back-compat alias → high_risk', () => {
...
test('🛑 errata v02: HANDOFF_PHRASINGS_VERBATIM exports all 4 variants verbatim', async () => {
...
如果你現在有立即的危險、請撥打 1925(安心專線、24 小時)。`);
  ...
  assert.match(HANDOFF_PHRASINGS_VERBATIM.high_risk, /你不需要『真的要做什麼』才能打/);
  // freeze
...
test('🛑 P0 Fix 3: kickoff trigger + mid-SOP at Step 4 → re-present Step 4 phrasing, no state advance', async () => {
  // A015 case: 學員打「我想自殺」 → engine-3 fast-path inits SOP at Step 4 →
  // 學員 disengages → comes back, frontend sends KICKOFF_TRIGGER_CONTENT.
  // crisis-sop must re-emit the Step 4 inject without advancing state.
  const sopState = {
    ...buildInitialSopState({ category: CRISIS_CATEGORY.PASSIVE_DW_STRONG, step1Variant: STEP1_VARIANT.C_1 }),
  ...
  // Synthetic kickoff text (matches api/chat.js KICKOFF_TRIGGER_CONTENT prefix).
  const r = await crisisSopHandler({
    session_state: state,
    user_response: '[session-start trigger — 本場第一輪、請依當前 phase_context...]',
  });
  assert.equal(r.handled, true);
  // No state advance (patch empty for sopState).
...
test('🛑 P0 Fix 3: kickoff + Step 3 (protective) mid-SOP → re-present protective inquiry', async () => {
  ...
  const r = await crisisSopHandler({
    session_state: state,
    user_response: '[session-start trigger — 本場第一輪...]',
  });
  assert.equal(r.handled, true);
...
test('🛑 P0 Fix 3: NON-kickoff user turn at Step 4 → normal dispatch (advances to Step 6)', async () => {
  // Regression: ensure Fix 3 only fires on kickoff, NOT on regular user response.
  // Student's actual reply progresses the SOP normally.
  const sopState = {
    ...buildInitialSopState({ category: CRISIS_CATEGORY.PASSIVE_DW_STRONG, step1Variant: STEP1_VARIANT.C_1 }),
    current_step: SOP_STEPS.STEP_4_HANDOFF,
    awaiting: CRISIS_AWAITING.HANDOFF_ACK,
  ...
  const r = await crisisSopHandler({
    session_state: state,
    user_response: '好',                    // real student ack, not kickoff
    user_profile: { passive_death_wish_count: 1 },
  });
  assert.equal(r.handled, true);
...
test('🛑 P0 Fix 3: kickoff at first-entry (no preExistingSopState) → NORMAL init + dispatch', async () => {
  // Edge: the very first turn that triggers crisis can ALSO be a kickoff
  // (unusual, defensive). Without preExistingSopState the resume path
  // should NOT fire — handler should initialize + dispatch normally.
  const state = {
    crisis_in_progress: true,
    primary_mode: 'crisis',
    // NO crisis_sop_state yet — first entry
    deep_signal_flags: { passive_dw_detected: true, passive_dw_signal: 'strong' },
  };
  const r = await crisisSopHandler({
    session_state: state,
    user_response: '[session-start trigger — kickoff]',
  });
  assert.equal(r.handled, true);
  // Initialize-then-dispatch path → state advances out of Step 1.

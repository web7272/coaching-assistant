...
export async function e4TakeawayHandler(ctx) {
  ...
  // ⭐ crisis_state_carry_forward write — triggered if this session had crisis.
  //   Step 6 will populate full schema; this batch writes the structural shell.
  ...
  let newCarryForward;
  if (prevCarry) {
    // Existing crisis — update auto-resolve counter.
    newCarryForward = updateCarryForwardOnSessionClose(prevCarry, crisisTriggeredThisSession);
  } else if (crisisTriggeredThisSession) {
    // Fresh crisis — build initial carry_forward.
    newCarryForward = buildCrisisCarryForward(state);
  } else {
    newCarryForward = null;   // no crisis, no carry_forward.
  }

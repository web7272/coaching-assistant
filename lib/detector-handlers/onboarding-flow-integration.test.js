...
//   3. Full 2-turn happy path (Vivi 6/7 2-step): !onboarded → step 1 →
//      step 2 → COMPLETE write (no step 3 confirm).
...
// ─── Full happy path (2 turns — Vivi 6/7) ────────────────

test('🛑 6/7 v5.2 onboarding full path: new student → step 1 → step 2 → COMPLETE (no step-3 confirm)', async () => {
  let state = {};
  ...
  // Turn 2: 學員 articulate「我跟先生的溝通」 → COMPLETE directly (no confirm).
  r = await onboardingFlowHandler({
    session_state: state, user_response: '我跟先生的溝通',
    student_context_onboarded: false,
  });
  // 6/7: completes same turn — no "Step 3 · Confirm" inject, no advance.
  assert.match(r.inject, /Onboarding Complete \(2-step, no confirm\)/);
  // Explicit "no 對嗎" guard (instruction to AI). The bare 「這樣對嗎?」
  // student-facing question shape must not appear.
  assert.match(r.inject, /不問「這樣對嗎」/);
  assert.doesNotMatch(r.inject, /Step 3 · Confirm/,
    'no Step 3 inject — 6/7 spec strips the confirm round-trip');
  // Acknowledge + Mode 1 first-question fused in same turn.
  assert.match(r.inject, /這 21 天我們從『我跟先生的溝通』開始/);
  assert.match(r.inject, /在『我跟先生的溝通』這塊、你想要什麼\?/);
  // State cleared (NOT advanced to step 3).
  assert.equal(r.patch.onboarding_step, null);
  // Write payload contract unchanged for chat.js atomic UPDATE.
...
// The previous reject-path test ("step 3 reject → step 2 → re-articulate →
// confirm") is removed (Vivi 6/7): step-3 confirm no longer exists, so the
// reject path also vanishes. If a learner wants to refine their framing,
// they refine via the Mode 1 first answer instead.
...
test('🛑 P0 Fix 2: stale flags WITHOUT sopState → onboarding does NOT defer (A015 brick fix)', async () => {
  ...
  // 6/7 2-step: clear articulate at step 2 → completes directly (no Step 3).
  assert.match(r.inject, /Onboarding Complete \(2-step, no confirm\)/);
  assert.equal(r.patch.onboarding_step, null);
});
...
// 6/6 step-3-related integration tests removed (Vivi 6/7 2-step).
// Their replacement — the step-2-to-COMPLETE path with category-correct
// acknowledge + Mode 1 first question — is locked at the unit test level
// (lib/detector-handlers/onboarding-flow.test.js) and by the full happy
// path test above.
...
test('🛑 errata v02: onboarding preemptive crisis defer → Step 4 direct-1925 phrasing (NOT abc)', async () => {
  ...
  // (6/6 wording 修訂: 「不繼續往前推」→「不急著往下走」 per Vivi 6/6.)
  assert.match(HANDOFF_PHRASINGS_VERBATIM.standard, /1925/);
  assert.match(HANDOFF_PHRASINGS_VERBATIM.standard, /不急著往下走/);
...
// ─── 6/7 Vivi widened vague: handler-level integration ──────────────────
//
// Patrick repro: with step-3 confirm gone (7617ac8), a soft vague like
// 「嗯不知道耶」 used to bleed through as the 21-day anchor. After the
// parseArticulate widening, the handler must re-prompt step 2 (NOT complete).

for (const vague of ['嗯不知道耶', '還在想', '都可以啦', '不太確定', '不曉得耶']) {
  test(`🛑 6/7 widened vague: step 2 + "${vague}" → re-prompt step 2 (NOT complete, NO write payload)`, async () => {
    const state = {
      onboarding_step: {
        current_step: ONBOARDING_STEPS.STEP_2_ARTICULATE,
        awaiting: ONBOARDING_AWAITING.ARTICULATE,
    ...
    const r = await onboardingFlowHandler({
      session_state: state, user_response: vague,
      student_context_onboarded: false,
    });
    // Re-prompt step 2 (NOT complete).
    assert.match(r.inject, /學員回應 vague — 輕引/);
    assert.doesNotMatch(r.inject, /Onboarding Complete/,
      `"${vague}" must NOT bleed through to COMPLETE (no garbage anchor)`);
    // State unchanged (still at step 2).
    assert.deepEqual(r.patch, {});
    // No write payload — chat.js must not UPDATE students with this junk.
    assert.equal(r.onboarding_complete_write, undefined,
      `"${vague}" must NOT emit onboarding_complete_write`);
  });
}

test('🛑 6/7 widened vague: real focus 「我不知道我要什麼但大概是工作」 → STILL completes (絕不誤殺)', async () => {
  // Counter-test to the over-blocking risk: 「我不知道...但大概是工作」 has
  // 實質 tail → parseArticulate returns it → handler completes normally.
  const state = {
    onboarding_step: {
      current_step: ONBOARDING_STEPS.STEP_2_ARTICULATE,
      awaiting: ONBOARDING_AWAITING.ARTICULATE,
  ...
  const r = await onboardingFlowHandler({
    session_state: state, user_response: '我不知道我要什麼但大概是工作',
    student_context_onboarded: false,
  });
  assert.match(r.inject, /Onboarding Complete \(2-step, no confirm\)/,
    '含 "不知道" fragment 但有實質尾巴 → 必須完成 (絕不誤殺)');
  assert.equal(r.onboarding_complete_write.active_context_name,
    '我不知道我要什麼但大概是工作');
  assert.equal(r.onboarding_complete_write.context_onboarded, true);
});

test('🛑 6/7 v5.2 onboarding: articulate > 30 chars at step 2 → name truncated + def 200 cap on complete', async () => {
  ...
  // Long articulate arrives at step 2 → completes directly (Vivi 6/7).
  const r = await onboardingFlowHandler({
    session_state: state, user_response: longText,
    student_context_onboarded: false,
  });

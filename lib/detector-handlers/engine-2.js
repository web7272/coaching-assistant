...
export async function e2MasterHandler(ctx) {
  ...
  // fable 6/26 P2 — E2 stay 上限:同一 candidate 連續 >=2 輪停在「要證據」→ 別再原地
  //   反覆追同一個證據,改角度(接回生活情境 / 往前推)。contained inject note,
  //   不硬跨引擎 reroute;counter 走 session_state。
  if (sub === 'stay') {
    const _cand = e2signal.candidate_term || state.current_quality_candidate_term || null;
    const _prev = state.last_stay_candidate || null;
    const _stayCount = (_cand && _prev && _cand === _prev)
      ? (Number(state.consecutive_stay_count) || 0) + 1
      : 1;
    patch.last_stay_candidate = _cand;
    patch.consecutive_stay_count = _stayCount;
    if (_stayCount >= 2) {
      inject = `${inject}\n\n---\n\n[SYSTEM INJECT — E2 stay 上限 (fable 6/26)]\n同一個候選特質「${_cand || '(這個)'}」已連續 ${_stayCount} 輪停在要證據。別再原地反覆追同一個證據——換角度:把它接回學員剛剛講過的生活情境(它在哪件事上其實已經發生過?),或往下一步推,而不是再要一次例子。`;
      patch.e2_stay_angle_switch_this_turn = true;
    }
  } else {
    patch.consecutive_stay_count = 0;
    patch.last_stay_candidate = null;
  }

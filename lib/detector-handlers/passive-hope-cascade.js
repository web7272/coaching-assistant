...
//
// Source 過渡期:
//   PR-23s4c: 用 Patch 23 PASSIVE_IMPLICIT_REGEX 當 interim source.
//   ⚠️ TODO(Step 5a): 引擎 1 S2 ship 後切換到 session_state.passive_hope_signals_detected
//      (引擎 1 偵測寫進 state, 本 handler 讀)。切換點寫在 PASSIVE_HOPE_SOURCE 註解.
//
// 邏輯 (per spec):
//   Step 1 context 評估:
...
import {
  PASSIVE_IMPLICIT_REGEX, SURFACE_LIFE_SIGNAL_REGEX,
...
// Hope-context regex per spec: 含「活下去 / 死亡 / 結束生命 / 此生 / 留戀 / 動力」
//   reuse SURFACE_LIFE_SIGNAL_REGEX (which includes most of these) +
//   add 死亡 / 結束生命 / 留戀.
const HOPE_CONTEXT_REGEX = /(活下去|死亡|結束生命|結束.{0,5}生命|此生|留戀|動力|想活|還活|意義)/;

// ⚠️ PASSIVE_HOPE_SOURCE switch — Step 5a 切換點:
//   PR-23s4c interim: read text + apply PASSIVE_IMPLICIT_REGEX inline.
//   Step 5a target:   read state.passive_hope_signals_detected (引擎 1 偵測).
function detectPassiveHopeFromText(text) {
  if (typeof text !== 'string' || text.length === 0) return false;
  return PASSIVE_IMPLICIT_REGEX.test(text);
}
...
export async function passiveHopeCascadeHandler(ctx) {
  ...
  // Step 1: passive hope source (TODO Step 5a switch to state read).
  const passiveHopeHit = detectPassiveHopeFromText(text);
  if (!passiveHopeHit) return { handled: false };

  // Step 1b: hope context check — must contain hope/life/death context words.
  const sessionTurns = Array.isArray(ctx.last_3_turns) ? ctx.last_3_turns : [];
  const turnsBlob = [text, ...sessionTurns.filter(t => typeof t === 'string')].join(' ');
  const hasHopeContext = HOPE_CONTEXT_REGEX.test(turnsBlob);
  if (!hasHopeContext) {
    // Pure passive hope without life-context — maintain as general passive_hope,
    // 不升級. (Per spec: 「否則維持一般 passive_hope、不升級」.)
    return { handled: false };
  }

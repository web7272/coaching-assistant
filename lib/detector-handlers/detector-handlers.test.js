...
// ─── 6/6 active SI hotfix — suicide classification (Patrick ground-truth) ─

test('🛑 6/6 hotfix: 「我想自殺」 → PASSIVE_STRONG hit (NOT pure trauma) → C-1 strong path', async () => {
  // PR-b root cause repro: pre-hotfix「我想自殺」only hit TRAUMA_REGEX (含「自殺」)
  // → 走變體 A trauma. With 6/6 patch, active SI 第一人稱 wins PASSIVE_STRONG first.
  const r = await e3DeepSignalHandler({
    session_state: {},
    user_response: '我想自殺',
    user_profile: { passive_death_wish_count: 0 },
  ...
  assert.equal(r.handled, true);
  assert.equal(r.patch.deep_signal_flags.passive_dw_detected, true,
    '「我想自殺」 must hit PASSIVE_STRONG (first-person active SI), not pure trauma');
  assert.equal(r.patch.deep_signal_flags.passive_dw_signal, 'strong');
  assert.equal(r.patch.deep_signal_flags.passive_dw_variant, 'strong');
  // Count bumps (passive DW counter, NOT trauma-only).
  assert.deepEqual(r.user_profile_increments, { passive_death_wish_count: 1 });
});

test('🛑 6/6 hotfix: 「我要自殺」 → PASSIVE_STRONG hit', async () => {
  const r = await e3DeepSignalHandler({
    session_state: {}, user_response: '我要自殺',
    user_profile: { passive_death_wish_count: 0 },
  });
  assert.equal(r.patch.deep_signal_flags.passive_dw_signal, 'strong');
});

test('🛑 6/6 hotfix: 「我要結束自己的生命」 → PASSIVE_STRONG hit (結束.{0,3}(自己|生命))', async () => {
  const r = await e3DeepSignalHandler({
    session_state: {}, user_response: '我要結束自己的生命',
    user_profile: { passive_death_wish_count: 0 },
  });
  assert.equal(r.patch.deep_signal_flags.passive_dw_signal, 'strong');
});

test('🛑 6/6 hotfix: 「我想死」 / 「我要死」 / 「活不下去」 → PASSIVE_STRONG hit', async () => {
  for (const text of ['我想死', '我要死', '活不下去']) {
    const r = await e3DeepSignalHandler({
      session_state: {}, user_response: text,
      user_profile: { passive_death_wish_count: 0 },
    });
    assert.equal(r.patch.deep_signal_flags.passive_dw_signal, 'strong',
      `${text} should hit PASSIVE_STRONG (first-person active SI)`);
  }
});
...
test('🛑 errata v02: e3DeepSignalHandler (A6) si_confirm_pending + 「沒有」 → DENY → standard direct-1925', async () => {
  ...
  // errata v02 direct-1925 phrasing (no 三選一).
  assert.match(r.inject, /Step 4 standard direct-1925 phrasing/);
  assert.match(r.inject, /這句話很重——我想停在這裡、不繼續往前推/);
...
test('🛑 errata v02: C-3 inject 改 Step 4 cumulative direct-1925 (廢 Vivi 6/4 三選一)', async () => {
  ...
  // errata v02 §10.3 step 4.2 verbatim phrasing in C-3 prompt.
  assert.match(r.inject, /過去幾次、你提過類似的/);
  assert.match(r.inject, /我想停在這裡、不繼續往前推/);
  assert.match(r.inject, /如果你現在有立即的危險、請撥打 1925/);

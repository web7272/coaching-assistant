...
test('🛑 buildTier3ReframeNote: 自由 → Freedom From vs To framing + R5 library body', () => {
  const inject = buildTier3ReframeNote('自由');
  assert.match(inject, /『自由』/);
  // Step 7 PR-7a: heading + ship-able fallback + R5 library full body.
  assert.match(inject, /Tier 3 Accept \+ R5 Reframe Library/);
  assert.match(inject, /Freedom From vs Freedom To Be/s);   // R5 body header
  assert.match(inject, /離開某個束縛.*朝向某個你想要的樣子/s);   // ship-able fallback
  assert.match(inject, /Away From.*Toward/s);                // library body title
  // R5 body 5-step verbatim anchors.
  assert.match(inject, /Step 2 — the flip/);
  assert.match(inject, /這個『不要 \[X\]』的背面/);
});

test('🛑 buildTier3ReframeNote: 真實 → 特殊 accept、no R5 invocation', () => {
  const inject = buildTier3ReframeNote('真實');
  assert.match(inject, /真實.*特殊/);
  assert.match(inject, /tier3_accepted_authenticity/);
  // 真實 = Damon 推薦項、直接 accept, NOT cascade R5 — assert R5 library body
  // (the 5-step script) is absent, not just the title.
  assert.doesNotMatch(inject, /Step 2 — the flip/);
  assert.doesNotMatch(inject, /這個『不要 \[X\]』的背面/);
});
...
test('🛑 e2MasterHandler: tier 3「自由」 → upgrade owned + reframe note appended', async () => {
  ...
  // Step 7 PR-7a: heading renamed to「Tier 3 Accept + R5 Reframe Library」.
  assert.match(r.inject, /Tier 3 Accept \+ R5 Reframe Library/);
  // R5 library body appended via buildTier3ReframeNote.
  assert.match(r.inject, /R5 Away From → Toward/);
});

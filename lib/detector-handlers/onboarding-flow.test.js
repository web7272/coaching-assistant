...
test('🛑 Vivi 6/19: step 1 自由文字「我想要留在美國」 → 暖化 re-prompt (收到 + 軟性遞回 5 選項, 不冷複製開場白)', async () => {
  const prev = buildInitialOnboardingState();
  const r = await onboardingFlowHandler({
    session_state: { onboarding_step: prev },
    user_response: '我想要留在美國',
    student_context_onboarded: false,
  });
  assert.equal(r.handled, true);
  assert.deepEqual(r.patch, {}, 'state unchanged on re-prompt');
  assert.match(r.inject, /收到/);
  assert.match(r.inject, /RE-PROMPT/);
  assert.match(r.inject, /比較接近你想探索的/);
  assert.match(r.inject, /1\. 事業 \/ 工作 \/ 金錢/);
  assert.match(r.inject, /5\. 自我 \/ 內在狀態 \/ 心理/);
  assert.match(r.inject, /輕推回選擇/);
  assert.match(r.inject, /必須輕推回落 1-5/);
  assert.equal(r.inject.includes('話術 (spec §1.2 verbatim'), false,
    '不再冷複製 STEP_1_INJECT verbatim 開場白');
  assert.equal(/處理的、比較接近/.test(r.inject), false);
});

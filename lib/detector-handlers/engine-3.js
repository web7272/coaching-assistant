...
// 6/12 Stage 2 — Containment judge for top1 PK (king-of-the-hill).
//   judge() takes pairwise (A, B, prior_question, user_response) → returns
//   containment_direction + linear_error + confidence. aPassesContainment
//   = A 是 containing 方 (高/中 confidence). 對偶 bPasses 自己組.
import {
  judge as containmentJudge,
  aPassesContainment,
} from '../haiku-judge/containment-logic.js';
...
//
// 6/12 Stage 2 — Top 1 PK king-of-the-hill state machine:
//   state.top1_pk_state = {
//     remaining:    string[],   // [0]=current king, [1]=challenger, ...rest queued
//     eliminated:   string[],   // 已淘汰 (append order; last = runner-up)
//     original_count: number,   // safety cap baseline
//     rounds:       number,     // judge call attempts (含 indecisive retry)
//   }
//   Handler 自己決定配對順序 (king-of-the-hill 單趟, 不解析 AI 問句).
//   top1Judge inject 動態 footer 帶 "本輪比 [A] vs [B]".
//   Containment judge 結果 → 淘汰 / 不淘汰 retry / linear error retry.
//   Safety cap: rounds > 2*original_count → handoff (M_LM_1 ambiguous 路徑).
//   PK 收斂 (remaining.length === 1) → top1_value 寫入 + transitionPrimary 切
//   IDENTITY_ANCHORING (Stage 2B 紅線, SAME patch 一起 emit).
// ─────────────────────────────────────────────────────────

// PK config constants — exposed for tests.
export const TOP1_PK_CONSTANTS = Object.freeze({
  MIN_LIST_FOR_TRIGGER: 3,
  SAFETY_CAP_MULTIPLIER: 2,   // safety_cap_rounds = SAFETY_CAP_MULTIPLIER * original_count
});

/**
 * Pure: build the parametrized footer appended to top1Judge.prompt_content.
 * Tells Sonnet which pair this turn's containment question targets.
 */
export function buildTop1PkPairFooter(pair, rounds, maxRounds) {
  const a = String(pair[0] || '');
  const b = String(pair[1] || '');
  return `\n\n---\n\n[本輪 PK by handler — king-of-the-hill]\n`
    + `本輪比 「${a}」 跟 「${b}」.\n`
    + `用 Step 3 的 containment 問句 (顛倒問兩次以方便 judge 抓 direction):\n`
    + `> 「沒有「${b}」, 「${a}」 還能存在嗎? 沒有「${a}」, 「${b}」 還能存在嗎?」\n\n`
    + `注意:\n`
    + `- 這輪由系統指定配對:「${a}」 是當前 king, 「${b}」 來挑戰.\n`
    + `- 學員回應後, Haiku containment judge 自動評估、淘汰一個、進下一輪.\n`
    + `- 不要自己挑下一對、不要做 N-1 全套 PK (handler 接管 pairing).\n`
    + `- 若學員出現 Linear Thinking Error, 照 Step 4 切 containment 解釋, 不算淘汰一輪.\n`
    + `- Round ${rounds + 1} of max ${maxRounds}.`;
}

/**
 * Pure: decide elimination based on containment judgment.
 *   'A' → A is the containing value, B eliminated
 *   'B' → B is the containing value, A eliminated
 *   null → no decision this round (retry: indecisive / linear error / unclear)
 */
export function decidePkElimination(judgment) {
  if (!judgment || typeof judgment !== 'object') return null;
  // linear thinking error → no eliminate, AI retries
  if (judgment.linear_thinking_error_detected) return null;
  // judgment didn't address containment (off-topic) → retry
  if (!judgment.answer_addresses_containment) return null;
  // A wins (helper enforces direction + confidence guards)
  if (aPassesContainment(judgment)) return 'A';
  // B wins (dual condition)
  if (judgment.containment_direction === 'B_contains_A'
      && (judgment.confidence === 'high' || judgment.confidence === 'medium')) {
    return 'B';
  }
  // interdependent / unclear / low confidence → retry
  return null;
}

/**
 * Pure: handoff inject when PK stalls (safety cap hit).
 * Reuses M_LM_1 ambiguous + handoff path (A006 protection — avoids 無限 PK
 * 反覆被拒挫敗).
 */
export function buildTop1PkHandoffInject(state) {
  const pk = state?.top1_pk_state || {};
  const remaining = Array.isArray(pk.remaining) ? pk.remaining.join(', ') : '';
  return `[SYSTEM INJECT — Top 1 PK stalled handoff (safety cap)]\n`
    + `Top 1 PK king-of-the-hill 跑了 ${pk.rounds || 0} 輪仍未收斂.\n`
    + `剩餘候選: ${remaining}\n\n`
    + `行動:\n`
    + `- current_quality_status = 'ambiguous' (對齊 M_LM_1 path).\n`
    + `- landmine_insistence_handoff = true → handoff_escalation 三選一.\n`
    + `- 不寫 top1_value, 不切 mode.\n`
    + `- spec §A2/A3 内含 handoff phrasing, Sonnet 走 ambiguous + handoff.`;
}
...
export async function e3Top1JudgeHandler(ctx) {
  ...
  const triggered = valuesCount >= TOP1_PK_CONSTANTS.MIN_LIST_FOR_TRIGGER
    && state.top1_value == null
    && isElicitationContext;

  if (!triggered) return { handled: false };

  const pkPrior = state.top1_pk_state;
  const isFirstTrigger = !pkPrior
    || !Array.isArray(pkPrior.remaining)
    || pkPrior.remaining.length < 2;

  // ─── First-time trigger: initialize PK state + ship inject ───
  if (isFirstTrigger) {
    const initial = [...state.values_collected_list];
    const pkState = {
      remaining: initial,
      eliminated: [],
      original_count: initial.length,
      rounds: 0,
    };
    const maxRounds = pkState.original_count * TOP1_PK_CONSTANTS.SAFETY_CAP_MULTIPLIER;
    const footer = buildTop1PkPairFooter(
      [pkState.remaining[0], pkState.remaining[1]],
      pkState.rounds,
      maxRounds,
    );
    return {
      handled: true,
      inject: top1Judge.prompt_content + footer,
      patch: {
        router_phase: 'top1_determination',
        top1_pk_state: pkState,
      },
    };
  }

  // ─── PK in progress: call containment judge on this turn's response ───
  const maxRounds = pkPrior.original_count * TOP1_PK_CONSTANTS.SAFETY_CAP_MULTIPLIER;
  const userResponse = ctx.user_response || '';
  const priorQuestion = state.last_ai_question || '';
  const pair = [pkPrior.remaining[0], pkPrior.remaining[1]];

  // Try judge call — empty user_response or empty prior_question → retry (no eliminate).
  let judgment = null;
  if (userResponse.trim() && priorQuestion.trim()) {
    try {
      const judgeFn = ctx.judges?.containment || containmentJudge;
      judgment = await judgeFn({
        user_response: userResponse,
        prior_containment_question: priorQuestion,
        values_being_compared: pair,
      });
    } catch (err) {
      // Judge throws (timeout / schema) → retry, no eliminate.
      if (err instanceof JudgeTimeoutError || err instanceof JudgeSchemaError) {
        ctx.logMiss?.({
          miss_type: 'judge_timeout',
          detector: 'E3_top1_pk_containment',
          error: err.message,
        });
        judgment = null;
      } else {
  ...
  const decision = decidePkElimination(judgment);
  const nextRounds = pkPrior.rounds + 1;

  // ─── Safety cap: too many rounds without convergence → handoff ───
  if (nextRounds > maxRounds) {
    return {
      handled: true,
      inject: buildTop1PkHandoffInject({
        ...state,
        top1_pk_state: { ...pkPrior, rounds: nextRounds },
      }),
      patch: {
        top1_pk_state: null,
        current_quality_status: 'ambiguous',
        landmine_insistence_handoff: true,
        top1_pk_stalled_at_round: nextRounds,
      },
    };
  }

  // ─── Apply elimination ───
  let nextRemaining = pkPrior.remaining;
  let nextEliminated = pkPrior.eliminated;
  if (decision === 'A') {
    // A (king) wins → eliminate B (challenger).
    nextEliminated = [...pkPrior.eliminated, pair[1]];
    nextRemaining = [pkPrior.remaining[0], ...pkPrior.remaining.slice(2)];
  } else if (decision === 'B') {
    // B (challenger) wins → eliminate A, B becomes new king.
    nextEliminated = [...pkPrior.eliminated, pair[0]];
    nextRemaining = [pkPrior.remaining[1], ...pkPrior.remaining.slice(2)];
  }
  // decision === null → no eliminate, retry next turn.

  // ─── Converged: 1 value remaining → write top1_value + transitionPrimary ───
  if (nextRemaining.length === 1) {
    const winner = nextRemaining[0];
    // values_ranking: winner first, runners-up by elimination order (last eliminated = rank 2).
    const valuesRanking = [winner, ...[...nextEliminated].reverse()];
    // 🔴 Stage 2B — SAME patch must transition primary_mode → identity_anchoring.
    const newModeState = transitionPrimary(modeRead, ACTIVE_MODES.IDENTITY_ANCHORING);
    const transitionEntry = buildTransitionEntry({
      from_active_modes: modeRead.active_modes,
      to_active_modes:   newModeState.active_modes,
      from_primary:      modeRead.primary_mode,
      to_primary:        newModeState.primary_mode,
      trigger_type:      TRIGGER_TYPES.MODE_NATURAL_PROGRESSION,
      trigger_detail:    'top1_value_determined_via_pk',
    ...
    return {
      handled: true,
      // No inject — top1 confirmed; engine-2 identity test will run next turn.
      inject: `[SYSTEM INJECT — Top 1 PK converged]\n`
        + `Top 1 = 「${winner}」 (king-of-the-hill 收斂, ${nextRounds} 輪).\n`
        + `values_ranking = ${JSON.stringify(valuesRanking)}.\n`
        + `primary_mode → identity_anchoring (engine-2 master 接手對 「${winner}」 做身份測試).`,
      patch: {
        ...newModeState,
        ...logPatch,
        top1_value: winner,
        values_ranking: valuesRanking,
        top1_pk_state: null,       // clear PK state
        router_phase: 'identity_test_routing',  // deprecated mirror per spec
      },
    };
  }

  // ─── Continue PK: next pair, parametrized inject ───
  const nextPkState = {
    remaining:      nextRemaining,
    eliminated:     nextEliminated,
    original_count: pkPrior.original_count,
    rounds:         nextRounds,
  };
  const nextFooter = buildTop1PkPairFooter(
    [nextRemaining[0], nextRemaining[1]],
    nextRounds,
    maxRounds,
  );
  return {
    handled: true,
    inject: top1Judge.prompt_content + nextFooter,
    patch: {
      router_phase: 'top1_determination',
      top1_pk_state: nextPkState,
    },
  };
}

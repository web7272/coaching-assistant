...
export async function crisisSopHandler(ctx) {
  ...
  //
  //   Detection: KICKOFF_TRIGGER_CONTENT begins with '[session-start trigger'
  //   (see api/chat.js export). Resume re-present only fires when sopState
  //   pre-existed AND turn is a kickoff (first-entry init still fires its
  //   own Step 1 dispatch on the triggering turn).
  const isKickoffTrigger = userResponse.startsWith('[session-start trigger');

...
export function selectSignalInject(signals, state = {}) {
  ...
  //   medium → R1 TODO (Step 7). strong + integration → integration-router 接管.
  if (hints.conditional_worth) {
    ...
    // medium/strong → flag only this turn (R1 / integration-router 接管).
  }
  // Priority 3 — S1 weak → flag only (R1 待 Step 7). medium/strong handled by R1.
  // No inject here.
  // Priority 4 — S5 in integration mode → ship-able「又」 inject.
  if (hints.negative_generalization && isIntegration) {
    return { inject: S5_INTEGRATION_INJECT, cascade: null };
  }
  // Priority 5 — S3 → interim「身份非成績單」 single-line inject (R7 待 Step 7).
  if (hints.frequency_illusion) {
    return { inject: S3_LIGHT_INJECT, cascade: null };
  }
...
export async function e1MasterHandler(ctx) {
  ...
  if (sub === 'none') {
    ...
    if (signalRouting.inject) {
      return {
        handled: true, inject: signalRouting.inject, patch: mergedPatch,
        user_profile_increments: signals.user_profile_increments,
      };
    }
    return {

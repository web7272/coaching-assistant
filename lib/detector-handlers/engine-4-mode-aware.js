...
// Step 6 PR-6b — Crisis SOP carry_forward full schema builder.
...
export function selectV6SubBranch(carryForward, passiveDwCount = 0) {
  ...
  // Priority 2 — si_risk_level surfaced.
  //   Accepts both turn2b spec enum ('passive' / 'active_no_plan' / 'active_with_plan')
  //   AND legacy values ('active' / 'high') for backward compat.
  const si = carryForward.si_risk_level;
  if (si === 'passive' || si === 'active_no_plan' || si === 'active_with_plan'
      || si === 'active' || si === 'high') {
...
/**
 * v5.1 Step 6 PR-6b — landing errata §3.2 V6 alignment.
 *   If V6 fires AND landing_page_reminder_delivered === true, V6 inject SHOULD NOT
 *   repeat the reminder (Sonnet already delivered it in prior crisis session).
 *   If false (e.g. session interrupted before Step 6), V6 inject SHOULD補 invoke.
 *
 * @param {object} carryForward
 * @returns {{shouldInjectReminderInV6: boolean, reason: string}}
 */
export function v6ReminderGate(carryForward) {
  if (!carryForward || typeof carryForward !== 'object') {
    return { shouldInjectReminderInV6: false, reason: 'no_carry_forward' };
  }
  if (carryForward.landing_page_reminder_delivered) {
    return { shouldInjectReminderInV6: false, reason: 'already_delivered_prior_session' };
  }
  return { shouldInjectReminderInV6: true, reason: 'not_yet_delivered_interrupted_prior' };
}
...
export const V6_SUB_BRANCH_PHRASING = Object.freeze({
  ...
  + '在台灣,1925 仍在那裡(安心專線、24 小時、免費),不需要『真的打算做什麼』才能打。\n'
 ...
 */
export function buildCrisisCarryForward(sessionState = {}, opts = {}) {
  ...
  // ⭐ v5.1 Step 6 PR-6b — Wire crisis_sop_state into full carry_forward.
  //   If SOP state machine has populated session_state.crisis_sop_state with
  //   step-by-step student answers, use buildFullCarryForward (sub-prompts/crisis)
  //   to derive the complete schema (turn2b §10.3 step 7.3 + landing errata §2.2).
  //   Caller passes opts.invokedAt to keep the function pure (no Date.now()).
  if (sessionState.crisis_sop_state && typeof sessionState.crisis_sop_state === 'object') {
    const full = buildFullCarryForward({
      sopState: sessionState.crisis_sop_state,
      invokedAt: opts.invokedAt || null,
    });
    if (full) {
      // Merge SOP-derived state with auto-resolve counter (Step 5c).
      return {
        ...full,
        // Mirror the legacy field for V6 sub-branch selection (Step 5c contract).
        handoff_choice: full.handoff_choice || sessionState.handoff_choice || null,
        sessions_since_trigger: 0,
      };
    }
  }

  // Fallback (Step 5c shell, pre-SOP) — keep behavior for tests / cases where
  // crisis triggers but SOP state machine hasn't populated yet (e.g. trigger turn
  // before crisis-sop dispatch).
  ...
  return {
    crisis_triggered_at: opts.invokedAt || null,
    crisis_category: category,
    handoff_choice: sessionState.handoff_choice || null,
    protective_factor_surfaced: null,
    si_risk_level:               null,
    safety_plan:                 null,
    next_session_focus:          null,
    // landing errata §2.2 — default false until SOP populates.
    landing_page_reminder_delivered: false,
    professional_referral_acknowledged: false,
 ...
 */
export function updateCarryForwardOnSessionClose(prevCarryForward, crisisTriggeredThisSession, opts = {}) {
  ...
  // ⚠️ M73 — Safety Planning 完成不得 reset resolved.
  //   This guard is enforced in chat.js Step 11+ wiring (caller never sets
  //   resolved_at on Safety Planning step); the only auto-resolve trigger is
  //   3 consecutive sessions WITHOUT re-trigger (natural_de_escalation).

  if (crisisTriggeredThisSession) {
    // ⚠️ M73 — re-trigger resets counter AND keeps resolved_at + resolution_type
    //   as null (explicitly, so spread doesn't leave them undefined).
    return {
      ...prevCarryForward,
      sessions_since_trigger: 0,
      resolved_at: prevCarryForward.resolved_at ?? null,
      resolution_type: prevCarryForward.resolution_type ?? null,
    };
  }
  ...
  if (next >= 3) {
    return {
      ...
      // ⚠️ caller-supplied ISO timestamp (no Date.now() — workflow resume safe).
      resolved_at: opts.invokedAt || null,

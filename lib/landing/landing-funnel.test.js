...
test('🛑 landing 7/15: quiz 預設可見 in .convert (無 openQuiz 二次點擊)', () => {
  // Vivi 7/15: 5 CTA #start → 直接落在問卷第一題,不再被 openQuiz 按鈕擋一層。
  assert.doesNotMatch(html, /id=["']openQuiz["']/);
  assert.doesNotMatch(html, /classList\.toggle\(['"]open['"]\)/);
  assert.match(html, /\.quiz\{display:block/);
});

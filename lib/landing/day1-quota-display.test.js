...
test('🛑 6/18 v8-V3: cta-d-3 內文換新 (verbatim「這不是聊天機器人。…你只需要活出真正的自己。」)', () => {
  const actionCard = html.match(
    /<div class="cta-card"[^>]*data-opt="3"[\s\S]*?<\/form>\s*<\/div>/,
  );
  assert.ok(actionCard);
  // v8 verbatim — Vivi 6/18 spec (含半形 space + 全形「、」「，」「。」「？」).
  assert.match(actionCard[0],
    /<p class="cta-d" id="cta-d-3">這不是聊天機器人。而是一段 21 天重新看見自己的旅程。看了很多書、聽了很多 Podcast、學了很多道理，怎麼還是沒有活出想要的人生？也許，你只差最後一哩路。不是再上一堂課。不是再學一個方法。而是停下來，好好跟自己對話。你不需要變成另一個人。你只需要活出真正的自己。<\/p>/);
  // Anti-regression: 舊 v7 內文「世界從不缺建議…活出屬於自己的答案。」 已廢.
  assert.equal(/世界從不缺建議。缺的是一場真正屬於自己的對話/.test(actionCard[0]), false,
    'v8-V3: 舊 v7 內文 (世界從不缺建議…) 應已被取代');

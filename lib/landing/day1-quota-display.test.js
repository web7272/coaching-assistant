...
test('🛑 6/18 v9-V4: cta-d-3 內文換新 (verbatim「這不是聊天機器人。…你只差最後一哩路。」)', () => {
  ...
  assert.match(actionCard[0],
    /<p class="cta-d" id="cta-d-3">這不是聊天機器人。而是一段 21 天重新看見自己的旅程。給自己一次機會，來跟教練對話。一起看見那些，你以為早已知道，卻從未真正理解的自己。活出想要的人生，也許，你只差最後一哩路。<\/p>/);
...
test('🛑 6/8 v9-②: 表單送出鈕 .cta-submit 文字「立刻領取 →」 (was「開始 →」)', () => {
  // 與其他 mid-page CTA「立刻領取 →」 一致 (G20 v2).
  // 只改顯示文字; type="submit" / class="cta-submit" / ctaSubmit(event,3) 流程不動.
  assert.match(html, /<button type="submit" class="cta-submit">立刻領取 →<\/button>/);

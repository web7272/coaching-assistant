...
test('🛑 6/8 v4 landing: branch "remaining > threshold" → 質性「限量體驗 - 每一次教練對話…需要花5秒鐘回答3個問題。」 verbatim', () => {
  // 6/8 v4 (Vivi 看 v3 live 後): 質性文案改短 (D4b).
  assert.match(html, /限量體驗 - 每一次教練對話都會產生成本/);
  assert.match(html, /需要花5秒鐘回答3個問題/);
...
test('🛑 6/15 landing: #cta-scarcity 搬進 action 卡 (data-opt="3") 內 (按鈕下方)', () => {
  // 6/15 Vivi invert: 止血線搬進卡內、按鈕下方、更明顯.
  ...
  assert.match(actionCard[0], /id="cta-scarcity"/,
    '#cta-scarcity 必須在卡內 (6/15 Vivi: 框內、按鈕下方)');
});

test('🛑 6/15 landing: #cta-scarcity 在按鈕之後、form 之前 (卡內、按鈕下方)', () => {
  // 6/15 Vivi: scarcity 搬進 .cta-trigger 內、緊接按鈕後、展開 form 前.
  const btnIdx      = html.indexOf('class="cta-button"');
  const scarcityIdx = html.indexOf('id="cta-scarcity"');
  const formIdx     = html.indexOf('class="cta-form"');
  assert.ok(btnIdx > 0 && scarcityIdx > 0 && formIdx > 0,
    'button / scarcity / form must all exist');
  assert.ok(btnIdx < scarcityIdx,
    `button (${btnIdx}) must precede scarcity (${scarcityIdx})`);
  assert.ok(scarcityIdx < formIdx,
    `scarcity (${scarcityIdx}) must precede form (${formIdx}) — 卡內、按鈕下方`);
});
...
test('🛑 6/8 v2/v3 + C: action card data-opt="3" still exists with form + 3-question quiz + new button class/text', () => {
  ...
  // Vivi 6/18: 表單抬頭 .cta-t-open「立刻領取《價值觀挖掘練習PDF》＋免費解鎖 AI 教練體驗」 已移除 (冗餘).
  assert.equal(/cta-t-open/.test(actionCard[0]), false, '表單抬頭 .cta-t-open 已移除');
  assert.equal(/立刻領取《價值觀挖掘練習PDF》＋免費解鎖 AI 教練體驗/.test(actionCard[0]), false,
    '表單抬頭文字 (立刻領取《...PDF》＋免費解鎖) 已不在 action card');
...
test('🛑 6/18 v8-D1: cta-t-3 換宣言標題「世界不缺答案。<br>缺的是一場真正屬於自己的對話。」 (Vivi 6/18)', () => {
  // Vivi 6/18: 卡片收合標題從「立刻領取《價值觀挖掘練習PDF》＋免費解鎖 AI 教練體驗」
  // 換成宣言式標題, <br> 斷兩行; 字型大小不變 (.cta-t clamp 不動, Vivi「用一樣的字型大小」).
  assert.match(html,
    /<p class="cta-t" id="cta-t-3">世界不缺答案。<br>缺的是一場真正屬於自己的對話。<\/p>/);
  // Anti-regression: 舊收合標題「立刻領取《價值觀挖掘練習PDF》<br>...」 已廢.
  assert.equal(/id="cta-t-3">立刻領取《價值觀挖掘練習PDF》/.test(html), false,
    '舊收合標題 (立刻領取《...PDF》) 已被宣言標題取代');
});
...
test('🛑 6/8 v7-V1: .cta-t 標題置中 (text-align:center) — 只影響標題, 不影響 .cta-d', () => {
  // 標題 2 行手動 <br> 斷行後, Vivi 看 live 想要置中. 內文 .cta-d 維持
  // 左對齊閱讀流 (Vivi spec: 「若 .cta-d 也想置中 Vivi 會另說」).
  const titleRule = html.match(/\.cta-card \.cta-t\s*\{[^}]*\}/);
  assert.ok(titleRule, '.cta-t CSS rule must exist');
  assert.match(titleRule[0], /text-align\s*:\s*center/);
  // Anti-regression: .cta-d 不應一併被置中 (Vivi 沒拍, 留左對齊閱讀流).
  const descRule = html.match(/\.cta-card \.cta-d\s*\{[^}]*\}/);
  assert.ok(descRule, '.cta-d CSS rule must exist');
  assert.equal(/text-align\s*:\s*center/.test(descRule[0]), false,
    '.cta-d 不該被加 text-align:center (Vivi 6/8 v7 只指 .cta-t)');
});

test('🛑 6/18 v9-V4: cta-d-3 內文換新 (verbatim「這不是聊天機器人。…你只差最後一哩路。」)', () => {
  const actionCard = html.match(
    /<div class="cta-card"[^>]*data-opt="3"[\s\S]*?<\/form>\s*<\/div>/,
  );
  assert.ok(actionCard);
  // v9 verbatim — Vivi 6/18 二改 (含半形 space + 全形「，」「。」).
  assert.match(actionCard[0],
    /<p class="cta-d" id="cta-d-3">這不是聊天機器人。而是一段 21 天重新看見自己的旅程。給自己一次機會，來跟教練對話。一起看見那些，你以為早已知道，卻從未真正理解的自己。<strong>活出想要的人生，也許，你只差最後一哩路。<\/strong><\/p>/);
  // Anti-regression: 舊 v7 內文「世界從不缺建議…」 + v8 內文「…你只需要活出真正的自己。」 皆已廢.
  assert.equal(/世界從不缺建議。缺的是一場真正屬於自己的對話/.test(actionCard[0]), false,
    'v9: 舊 v7 內文 (世界從不缺建議…) 應已被取代');
  assert.equal(/你只需要活出真正的自己/.test(actionCard[0]), false,
    'v9: 舊 v8 內文尾句 (你只需要活出真正的自己) 應已被取代');
  // Anti-regression: v4 舊內文「想像一下…我們陪你找回來。」 已廢.
  assert.equal(/想像一下,當你不再把能量耗在/.test(actionCard[0]), false,
    '6/8 v7-V2: v4 舊內文 開頭「想像一下…」 應已被取代');
  assert.equal(/那個你一直都在/.test(actionCard[0]), false,
    '6/8 v7-V2: v4 舊內文 中段「那個你一直都在」 應已廢');
  assert.equal(/我們陪你找回來/.test(actionCard[0]), false,
    '6/8 v7-V2: v4 舊內文 尾句「我們陪你找回來」 應已廢');
  // Anti-regression: v3 舊內文 (早期殘留) 仍須無.
...
test('🛑 6/8 C2: .cta-button CSS rule = 設計師 verbatim (粉 #F3D8CE / #7A361F / flex gap 9)', () => {
  ...
  assert.match(rule[0], /gap\s*:\s*3px/);
  // hover + active states.
...
test('🛑 6/8 C3: .cta-subline CSS rule = 設計師 verbatim (#B08968 + 12.5px + 置中)', () => {
  ...
  assert.match(rule[0], /margin-top\s*:\s*0/);
  assert.match(rule[0], /text-align\s*:\s*center/);
  assert.match(rule[0], /font-size\s*:\s*12px/);
  assert.match(rule[0], /color\s*:\s*#7A361F/);
...
test('🛑 6/15 hero: h1 新文案「別找答案，找你自己。」(br before glow)', () => {
  assert.match(html,
    /<h1 class="htag">別找答案，<br><span class="glow">找你自己<\/span>。<\/h1>/);
});
...
// ─── 6/8 v8 — quiz 小標併一行 + 三問句加底色 (Vivi 拍板) ────────────

test('🛑 6/8 v8-W1: .cta-quiz-head span 改 inline (was display:block) — 小標併一行', () => {
  // 找這個 selector 的整條 rule (要避免 ::before pseudo).
  const rule = html.match(/\.cta-quiz-head span\s*\{[^}]*\}/);
  assert.ok(rule, '.cta-quiz-head span CSS rule must exist');
  assert.match(rule[0], /display\s*:\s*inline\b/);
  // Anti-regression: 舊 display:block 已廢.
  assert.equal(/display\s*:\s*block/.test(rule[0]), false,
    '6/8 v8-W1: 舊 display:block 已換成 inline');
});

test('🛑 6/8 v8-W1: .cta-quiz-head span 有分隔 (「—」 ::before 或等效) 讓兩段順', () => {
  // Patrick spec: 「前後可補一個空格或「—」分隔讓它順」.
  // 以 ::before 注入「—」 是最乾淨的做法 — 文字 0 改 (span 內容不動).
  assert.match(html, /\.cta-quiz-head span::before\s*\{[^}]*content\s*:\s*["']—\s*["'][^}]*\}/);
});

test('🛑 6/8 v8-W2: .cta-qt 加底色 + padding + 圓角 (Patrick verbatim 值)', () => {
  const rule = html.match(/\.cta-q \.cta-qt\s*\{[^}]*\}/);
  assert.ok(rule, '.cta-q .cta-qt CSS rule must exist');
  // verbatim spec 值.
  assert.match(rule[0], /background\s*:\s*rgba\(\s*214\s*,\s*130\s*,\s*106\s*,\s*\.10\s*\)/);
  assert.match(rule[0], /padding\s*:\s*6px\s+10px/);
  assert.match(rule[0], /border-radius\s*:\s*6px/);
  // 既有 v6 grid-column:1/-1 + font-weight:500 etc 不能被覆蓋掉.
  assert.match(rule[0], /grid-column\s*:\s*1\s*\/\s*-1/);
  assert.match(rule[0], /font-weight\s*:\s*500/);
});

// ─── 6/8 v9 — 內文「生。」孤字修 (Vivi 6/8) ────────────────────────

test('🛑 6/8 v9: .cta-d 加 text-wrap:pretty 防尾字孤行 (CJK 逐字斷行 fallback safe)', () => {
  // text-wrap:pretty 讓瀏覽器自動重分配最後一行 (尾巴「生。」不再孤字);
  // 不支援的舊瀏覽器照常 wrap, 不會壞.
  const rule = html.match(/\.cta-card \.cta-d\s*\{[^}]*\}/);
  assert.ok(rule, '.cta-card .cta-d CSS rule must exist');
  assert.match(rule[0], /text-wrap\s*:\s*pretty/);
  // 既有 v4 / v7 規則 (font-family/font-size/color/line-height/margin) 不能
  // 被覆蓋掉 — v9 只 append 一條 declaration.
  assert.match(rule[0], /font-family\s*:\s*var\(--serif\)/);
  assert.match(rule[0], /font-size\s*:\s*14px/);
  assert.match(rule[0], /line-height\s*:\s*1\.75/);
  assert.match(rule[0], /margin\s*:\s*0\s+0\s+14px/);
});

test('🛑 6/18 v9-③: 表單送出鈕 .cta-submit 文字「開始對話 →」 (was「立刻領取 →」)', () => {
  // Vivi 6/18: 表單在「來跟教練對話」框架下, 送出鈕改「開始對話 →」.
  // 只改顯示文字; type="submit" / class="cta-submit" / ctaSubmit(event,3) 流程不動.
  assert.match(html, /<button type="submit" class="cta-submit">開始對話 →<\/button>/);
  // Anti-regression: 舊送出鈕文字「立刻領取 →」 已換 (4 mid-page ghost CTA 仍保留「立刻領取 →」, 見 G20).
  assert.equal(/<button type="submit" class="cta-submit">立刻領取 →<\/button>/.test(html), false,
    'v9-③: 舊 .cta-submit 文字「立刻領取 →」 已換成「開始對話 →」');
  // Anti-regression: 舊「開始 →」 已廢.
  assert.equal(/<button[^>]*class="cta-submit"[^>]*>開始 →</.test(html), false,
    'v9-②: 舊 cta-submit 按鈕文字「開始 →」 已換成「立刻領取 →」');
});

test('🛑 6/8 v9-② anti-regression: 不能誤改展開鈕 .cta-button「開始看見自己 →」', () => {
  // .cta-button (id="cta-go-3") 是動作卡「點擊展開」 那顆 (C 段粉色),
  // 跟 .cta-submit (送 option-3 form) 完全不同 — v9-② 只動 submit.
  const actionCard = html.match(
    /<div class="cta-card"[^>]*data-opt="3"[\s\S]*?<\/form>\s*<\/div>/,
  );
  assert.ok(actionCard);
  // 展開鈕 0 改, 仍保留 v6/C 文字.
  assert.match(actionCard[0], /<p class="cta-button" id="cta-go-3"><span class="cta-button-main"><span>開始看見自己<\/span>/);
});

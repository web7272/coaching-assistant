...
test('🛑 6/8 v7-V2: cta-d-3 內文換新 (verbatim「該你了。我們製作了一份PDF…聚焦在你想要的人生。」)', () => {
  const actionCard = html.match(
    /<div class="cta-card"[^>]*data-opt="3"[\s\S]*?<\/form>\s*<\/div>/,
  );
  assert.ok(actionCard);
  // v7 verbatim — Patrick 6/8 spec (含半形 space + 全形「，」「。」).
  assert.match(actionCard[0],
    /<p class="cta-d" id="cta-d-3">世界從不缺建議。缺的是一場真正屬於自己的對話。有時候，我們吸收了太多別人的金句，看了很多書，聽了很多 Podcast，卻忘了從來不是資訊不夠，而是太多答案，讓你聽不見自己的聲音。透過PDF跟 AI 教練對話，幫助你釐清價值觀，活出屬於自己的答案。<\/p>/);

...
test('🛑 6/8 v3 landing: #cta-scarcity 在按鈕 (.cta-button「開始看見自己」) 之前 (推力最大時刻)', () => {
  // 6/8 C 段 update — 按鈕 class 由 cta-go--btn → cta-button, 文字 兩個都要 → 開始看見自己.
  const actionCard = html.match(
    /<div class="cta-card"[^>]*data-opt="3"[\s\S]*?<\/form>\s*<\/div>/,
  );
  ...
  assert.ok(btnIdx > 0,      '.cta-button must exist in action card');
  assert.ok(scarcityIdx < btnIdx,
    `#cta-scarcity (idx ${scarcityIdx}) must come before .cta-button (idx ${btnIdx})`);
});
...
test('🛑 6/8 C3: subline element 在按鈕正下方 (class=cta-subline, verbatim 文字)', () => {
  const actionCard = html.match(
    /<div class="cta-card"[^>]*data-opt="3"[\s\S]*?<\/form>\s*<\/div>/,
  );
  assert.ok(actionCard);
  // Patrick 6/8 spec verbatim (含全形「＋」 U+FF0B).
  assert.match(actionCard[0],
    /<span class="cta-subline">免費領 PDF ＋ 體驗 Day 1<\/span>/);
});
...
test('🛑 6/8 C — 卡片堆疊 verbatim: 標題 → 內文 → scarcity → 按鈕 → subline', () => {
  const actionCard = html.match(
    /<div class="cta-card"[^>]*data-opt="3"[\s\S]*?<\/form>\s*<\/div>/,
  );
  assert.ok(actionCard);
  ...
  assert.ok(descIdx < scarcityIdx,
    `desc (${descIdx}) must come before scarcity (${scarcityIdx})`);
  assert.ok(scarcityIdx < btnIdx,
    `scarcity (${scarcityIdx}) must come before button (${btnIdx}) — 推力最大時刻`);
  assert.ok(btnIdx < sublineIdx,
    `button (${btnIdx}) must come before subline (${sublineIdx}) — subline 在按鈕之下`);
});

...
test('🛑 PR-23s4c task 2: 生活場景化 intent moved to integration-router prompt', async () => {
  // PR-23s4b: transitions/ 廢除. PR-23s4c: integration-router 接管 toolbox.
  // 生活場景化 (跟誰 / 做哪幾件事 / 選哪個方向) intent verified in integration-router.
  // 原 transitions/phase-3b-to-3a-simplified 的紅線 14 / dissociated→associated 精確化
  // intent 也由 integration-router toolbox + 紅線 14 (cached damon-core) 共同承擔.
  const { default: integrationRouter } = await import('../engine-3/integration-router.js');
  assert.match(integrationRouter.prompt_content, /Scope Overlap.*生活場景|跟誰見面.*做哪幾件事.*選哪個方向/s);
  // 紅線 14 紙寫在 cached damon-core (single source of truth, not duplicated here).
  assert.match(integrationRouter.prompt_content, /不主動問身體 \(Scope Overlap default 不問 sub-modality, 紅線 14\)/);
});
...
test('🛑 PR-23s4c task 2: transitions/ 廢除後 owned_via_acceptance 由 mode-transition-router 接管', async () => {
  // PR-23s4b retired transitions/. PR-23s4c integration-router + mode-transition-router
  // jointly cover the routes:
  //   - 3b→4-acceptance (owned_via_acceptance): mode-transition-router transition + integration-router framing.
  //   - 3a→3b-regression: now mode-flexible (no regression concept in v5.1).
  //   - 3b→3a-simplified: integration toolbox dynamic selection.
  const { default: modeTransitionRouter } = await import('../engine-3/mode-transition-router.js');
  const { default: integrationRouter } = await import('../engine-3/integration-router.js');
  // mode-transition-router prompt mentions owned_via_acceptance transition.
  assert.match(modeTransitionRouter.prompt_content, /owned_via_acceptance/);
  // integration-router prompt mentions owned_via_acceptance exit path.
  assert.match(integrationRouter.prompt_content, /owned_via_acceptance/);
  assert.match(integrationRouter.exit_conditions.to_cascade, /owned_via_acceptance/);
});

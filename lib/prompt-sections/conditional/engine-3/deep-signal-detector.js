...
//     累積觸發三選一; C-2 後若學員答「字面 / 哲學」走 light 1925 不開三選一,
//     per Vivi 6/4 折衷裁決 — H4 漏判風險由 dashboard 連 3 次 caution 監控).
//
...
// 禁止段 6 條 (Vivi 6/4 patch 原檔 5 條 + 保留 PR-23a 第 5 條淺安撫禁令為第 6).
const FORBIDDEN_6 = `**禁止**:
- 不可把 passive 訊號當「哲學表述 / 隱喻」滑過——A006 Day 1 教訓
- 不可在 passive 訊號 surface 後、繼續挖 values 找 Top 1
- 不可假設「學員講出來 = 已被處理」——處理是 handoff + 1925、不是「聽到了 = 解決了」
- 不可在累積觸發 ≥ 3 時、仍提供 (c) 選項
- passive_death_wish_count >= 5(跨 session 累積):強制 HITL alert Vivi、freeze AI 推進、轉介
...
**機制**:
- patch 設 si_confirm_pending=true → 下一輪 handler 走 SI 分類路徑.
  TODO(任務3): 替換為 Crisis Mode SI SOP (full risk assessment + safety plan).
...
// 變體 C-3: 累積 ≥ 3 次 (Vivi 6/4 patch 原檔, 移除 (c)).
...
**話術 — 承認段 (Vivi 6/4 patch 原檔)**:

> 「我聽到了。
> 過去幾次對話、你提過類似的——
> 我必須跟你直接說:這個我沒辦法在這裡輕輕帶過。
>
> 我想問你:今天、現在這個時刻——你安全嗎?」

**三選一 (移除 c, Vivi 6/4 patch 原檔)**:
...
> 你選哪個都可以——我不繼續推進。
>
> (台灣的 1925、24 小時都在、不需要『真的要做什麼』才能打。)」
...
// 變體 C-4: count >= 5 → freeze (Vivi 6/4 patch 原檔 + 6/4 spec 第 5 條禁止段映射).

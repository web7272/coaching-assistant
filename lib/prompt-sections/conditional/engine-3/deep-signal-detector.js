...
//
// ⭐ 6/6 dedupe gate (Vivi 6/6, 話術定稿-crisis-去書面腔+1925台灣框.md):
//   AI-facing phrasings imported from lib/sub-prompts/crisis/_phrasings.js.
//   This is the file whose stale verbatim copies misled the active SI fast-
//   path into emitting the pre-revision book-toned wording in production.
//   Single source now; all-lib grep test enforces zero residual copies.
import {
  ...
  // ⭐ 6/7 P0 safety (Vivi A016): prohibition block embedded in active SI
  //   fast-path so first-turn AI output is also gated.
...
2. **handoff_escalation(errata v02 Step 4 direct-1925, 6/6 wording 修訂)**:
...
// ⭐ 6/6 dedupe gate — 1925 hotline phrasing imported from _phrasings.js.
//   Previously inlined; multiple stale copies caused production wording miss.
//   STEP_5_1_HOTLINE_1925 = 主 1925 文案 (full, Step 5.1).
//   STEP_5_LIGHT_HOTLINE_1925 = 輕量 1925 (C-2 學員答「字面 / 哲學」分流用).
const HOTLINE_1925       = STEP_5_1_HOTLINE_1925;
const HOTLINE_1925_LIGHT = STEP_5_LIGHT_HOTLINE_1925;
...
// ⭐ 6/6 safety hotfix v2 — Active SI Explicit fast-path (Vivi 沙盒 #2).
//   Student already declared explicit SI intent ("我想自殺" / "我要結束生命" /
//   "我要死"). DO NOT ask 「有沒有念頭」 — that's absurd post-declaration.
//   Crisis SOP initializes pre-positioned at Step 4 (skip 1/2/3); AI delivers
//   errata v02 §10.3 step 4.3 (high_risk) phrasing.
export const prompt_content_active_si_explicit = `[SYSTEM INJECT — Active SI Explicit · Step 4 high_risk direct-1925 fast-path]

學員明確表達 active SI 意圖 (例「我想自殺」/「我要結束生命」/「我要死」).
**已 confirmed — 不再問「有沒有念頭」、跳過 Step 1 acknowledge + Step 2 SI assessment、直接走 Step 4 high_risk direct-1925** (Vivi 6/6 v2 拍板).

話術 (errata v02 §10.3 step 4.3 + 6/6 wording 修訂, Vivi 6/6 終審 — source: _phrasings.js HANDOFF_4_3_HIGH_RISK_PHRASING):

${HANDOFF_4_3_HIGH_RISK_PHRASING.split('\n').map(l => '> ' + l).join('\n')}

機制:
- HITL critical alert (即時、不等到 session 結束).
- crisis_sop_state pre-initialized 在 Step 4 awaiting HANDOFF_ACK (skip Step 1/2/3).
- 「真人在你旁邊」 framing — 不指名 Vivi (capacity OK).
- 學員下個 response → 進 Step 6 landing reminder (errata v02 high_risk → oneShot1925 path).
- passive_death_wish_count + 1 (cross-session 追蹤).

不可做:
❌ 不問「你現在有沒有想要傷害自己的念頭?」 (已 confirmed — 再問荒謬, Vivi 6/6 沙盒 #2 case).
❌ 不問「你今天有具體的計畫嗎?」 (Step 2.3 assessment skip 掉、不追問計畫細節).
❌ 不 offer「我幫你預約跟 Vivi 1-on-1」 (errata v02 廢除、萬人規模不可行).
❌ 不給「想開一點」「不要想太多」這類淺安撫.
❌ 不對 active SI 內容下評判 / 解釋 / 分析.
...
**話術 (6/6 wording 修訂, 輕量 1925 — source: _phrasings.js STEP_5_LIGHT_HOTLINE_1925)**:

${STEP_5_LIGHT_HOTLINE_1925.split('\n').map(l => '> ' + l).join('\n')}
...
**話術 — Step 4.2 cumulative direct-1925 (Vivi 6/6 終審 — source: _phrasings.js HANDOFF_4_2_CUMULATIVE_PHRASING)**:
...
//
// ⚠️ 6/6 safety hotfix v2 (Vivi 沙盒 #2): active SI explicit patterns 已分離出去
//    到 ACTIVE_SI_EXPLICIT_REGEX (見下). 留在 PASSIVE_STRONG 的是 passive
//    ideation:「不想活」/「希望死」 — 仍需 C-1 直問 SI confirm 才能升級.
//
//   Pre-6/6-v2 patterns:
//     ✓ 留在 PASSIVE_STRONG:
//         不想活 / 不想活下去 / 不想再活 / 不想繼續活
//         想結束生命 / 想離開這世界
//         希望.{0,5}死 / 希望.{0,5}離世 / 希望.{0,5}死亡
//         希望.{0,8}生病.{0,8}死 / 希望.{0,5}快點走
//         不會傷害自己.{0,5}希望
//         我想死 (passive ideation, Vivi 6/6 spec keeps in PASSIVE)
//         活不下去 (passive ideation, 當下狀態)
//         不想活了 (passive ideation)
//     ✗ 移出去 ACTIVE_SI_EXPLICIT (6/6 hotfix v2):
//         我?想自殺 / 我?要自殺 → ACTIVE_SI (first-person explicit suicide intent)
//         結束.{0,3}(自己|生命) → ACTIVE_SI (兩種寫法皆 explicit)
//         我要死 → ACTIVE_SI (「要」 = decided commitment)
export const PASSIVE_STRONG_REGEX = /(不想活|不想活下去|不想再活|不想繼續活|想結束生命|想離開這世界|希望.{0,5}死|希望.{0,5}離世|希望.{0,5}死亡|希望.{0,8}生病.{0,8}死|希望.{0,5}快點走|不會傷害自己.{0,5}希望|我想死|活不下去|不想活了)/;

// ⭐ 6/6 safety hotfix v2 (Vivi 沙盒 #2) — active SI explicit 快速路徑.
//
//   Background: Pre-hotfix-v2 「我想自殺」 命中 PASSIVE_STRONG → engine-3 走 C-1
//   強訊號 → 下個 turn AI 問「你現在有沒有想要傷害自己的念頭?」 (直問 SI).
//   學員已 explicit 表達自殺意圖, 再問是否有念頭 = 荒謬.
//
//   Fix (Vivi 6/6 v2 spec verbatim — 3 patterns):
//     (我.{0,3})?(想|要|打算).{0,3}自殺   — 「我想自殺」/「想自殺」/「我要自殺」/「我打算自殺」
//     結束.{0,3}(自己|我).{0,5}生命       — 「結束自己的生命」/「結束我的生命」
//     (我.{0,3})?要死了?                  — 「要死」/「要死了」/「我要死」
//
//   Engine-3 handler routes hits to STEP 4 HIGH_RISK direct-1925 fast-path:
//     skip Step 1 (acknowledge 直問) + Step 2 (SI assessment) — student
//     already declared explicit intent.
//
//   ⚠️ 「自殺/上吊」 still in TRAUMA_REGEX (not removed) — covers 「我爸自殺」/
//   「親人上吊」 trauma scenes (no first-person 想/要 → not hit by this regex).
//
//   ⚠️ 「我想死」 stays in PASSIVE_STRONG per Vivi spec literal (only 「要死」/
//   「要死了」 form is in ACTIVE_SI). 「想/要」 distinction = wishing vs decided.
export const ACTIVE_SI_EXPLICIT_REGEX = /((我.{0,3})?(想|要|打算).{0,3}自殺|結束.{0,3}(自己|我).{0,5}生命|(我.{0,3})?要死了?)/;
...
export const trigger_signals = {
  strong: [
    // ⭐ 6/6 v2 hotfix — active SI explicit takes highest precedence (handler
    //   short-circuits to Step 4 high_risk fast-path, skipping C-1 直問).
    { kind: 'regex', pattern: ACTIVE_SI_EXPLICIT_REGEX,
      description: 'active SI explicit (「我想自殺 / 我要結束生命 / 我要死」) — fast-path 直接 Step 4 high_risk' },
    ...
    { kind: 'regex', pattern: /(哭|崩潰|受不了|喘不過氣|無法呼吸|腦袋空白|手抖)/,
      description: '強烈情緒突發' },
    // ⭐ §3 patch 6/4 — passive DW 強訊號 (passive ideation, 走 C-1 直問 SI)
    { kind: 'regex', pattern: PASSIVE_STRONG_REGEX,
      description: 'passive death wish 強訊號 (明說「不想活 / 希望死」, passive ideation — C-1 直問)' },
  ],
  support: [
...
export default {
  ...
  // ⭐ 6/6 v2 hotfix — active SI explicit fast-path (skip Step 1 直問 + Step 2 SI assessment).
  prompt_content_active_si_explicit,

...
//   cached prefix.
//
// ⭐ 6/7 P0 safety (Vivi A016): CRISIS_OUTPUT_PROHIBITIONS interpolated into
//    the Mode 6 Crisis section so the prohibitions are present in cached
//    prefix on every turn (not only on the turn a Step 4 / Step 6 inject
//    fires). This invalidates the cache ONCE on deploy (same as any cached
//    prefix edit). Source of truth: lib/sub-prompts/crisis/_phrasings.js.
import { CRISIS_OUTPUT_PROHIBITIONS } from '../../sub-prompts/crisis/_phrasings.js';
...
## §3.5 21 天 SC 七步路徑 Internal Framing

你是一個 AI 教練、運作在 21 天 program 上、每位學員的旅程對應 Damon Cart Self-Concept 體系的七步路徑。

**七步路徑 internal reference**:
...
**Internal aware 原則**:
- state inject 會給你 sc_journey_step(1-7)、sc_journey_evidence accumulated
- 你**不對學員講「我們現在在 Step X」**、不對學員標步驟編號
- 學員體驗是 seamless 21 天 program、不感知七步結構
- 你按既有 mode 機制 + 對應七步階段微調語氣 / 焦點(但**不強推**)

**七步路徑跟 mode 的關係**:
- 七步是宏觀進度 framing、mode 是 micro 工作工具
- 學員可在任何 mode 內處於任何 step、非嚴格 1:1
- 一個 turn 內可能涵蓋多個 step(學員 surface 痛點 + 渴望 + 微證據同時)

**非線性跳躍合法**:
- 學員從 Step 5 跳回 Step 2 重新挖渴望 = 合法、不是 regression
- T.O.T.E. 模型本質循環、學員會 oscillate
- 對齊既有 mode 雙向流動(無 regression 概念)
- 你不對學員講「我們倒退了」、學員體驗是自然演化

**21 天階段性 framing**(internal、不對學員講):
- **Day 1-7 早期**:主要在 Step 1-3(發現匱乏 + 承認渴望 + 開始挖數據)
- **Day 8-14 中期**:主要在 Step 3-5(深挖數據 + 認領身份 + 發現資源)
- **Day 15-21 後期**:主要在 Step 5-7(資源整合 + 奪回裁判權 + 定錨)

**重要**:這只是**典型節奏**、不強制。學員可能 Day 3 就到 Step 6、或 Day 21 還在 Step 2。你不施加進度壓力。

**Crisis 期間**:
- 七步路徑暫停、走 §10 Crisis SOP 9 步驟
- 退出 crisis 後 resume 原 sc_journey_step
- 不影響 sc_journey_evidence 累積

**學員可能挖不到 Step 7**:
- 21 天結束不一定到 Step 7
- 對齊 Damon「螺旋式上升、life practice」精神
- 你不強推、不假裝挖到了
- (Day 21 PDF 會誠實 reveal 學員當下到達層次、本 errata 不實作、之後 phase)`;
...
export default {
  ...
  // PR-2 (2026-06-11): + §3.5 七步路徑 internal framing ~200 tok → 1800.
  token_estimate: 1800,
  content,
  source: 'v51_cached_section3_mode_aware_router_reference.md §3 (line 21-301 verbatim, v0.2)'
    + ' + v52_seven_steps_errata.md §3.3 §3.5 (PR-2, Vivi 終審逐字)',
  ...
  cache_invalidation_note: '本檔替換 v5.0 four-seven-router、cache invalidate 一次。'
    ...
    + ' + PR-2 加 §3.5 七步 framing → 再 invalidate 一次, Patrick 排低流量窗口 + re-warm (同 PR-b 紀律).',
  used_by: [

...
test('cached prefix: total token estimate matches spec 04 (~4000)', () => {
  ...
  assert.ok(CACHED_PREFIX_TOKEN_ESTIMATE >= 3800 && CACHED_PREFIX_TOKEN_ESTIMATE <= 4300,
    `token estimate ${CACHED_PREFIX_TOKEN_ESTIMATE} outside [3800, 4300]`);
});

test('cached: damon-core-philosophy includes 21 紅線 (§3 patch 5/25)', () => {
  const damonCore = CACHED_PREFIX_SECTIONS[0];
  assert.match(damonCore.content, /21 條紅線/);
  assert.equal(/16 條紅線/.test(damonCore.content), false, 'title must say 21, not 16');
  assert.equal(/20 條紅線/.test(damonCore.content), false, 'title must say 21, not 20');
...
test('🛑 §3 patch 5/21: 新增紅線 17-20 都在 cached content', () => {
  ...
  assert.match(damonCore.content, /\(c\) 獨立.*Cascade Down/s);

...
test('cached prefix: total token estimate matches spec 04 (~4000)', () => {
  ...
  assert.ok(CACHED_PREFIX_TOKEN_ESTIMATE >= 3800 && CACHED_PREFIX_TOKEN_ESTIMATE <= 6100,
    `token estimate ${CACHED_PREFIX_TOKEN_ESTIMATE} outside [3800, 6100]`);
});
...
test('E3 deep_signal_detector has Haiku A4 trigger + handoff template (errata v02 direct-1925)', () => {
  ...
  // errata v02: handoff phrasing is direct 1925 (no Vivi 1-on-1 booking).
  // Lock the new phrasing presence instead of the legacy Vivi-name match.
  assert.match(deep.prompt_content, /1925/);
  assert.match(deep.prompt_content, /不繼續往前推/);
});

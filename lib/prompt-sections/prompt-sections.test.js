...
test('cached prefix: total token estimate matches spec 04 (~4000)', () => {
  ...
  // 6/26 (fable 審查 · Vivi 審過, 紅線21 疲乏豁免+見證輪 + 核心句式一主三變):
  //   damon 2400→2680 (+280) → total 6370→6650 → ceiling 6550→6750 留 100 tok 緩衝.
  //   ⚠️ cache invalidation 再一次 (排低流量窗口 + re-warm).
  assert.ok(CACHED_PREFIX_TOKEN_ESTIMATE >= 3800 && CACHED_PREFIX_TOKEN_ESTIMATE <= 6750,
    `token estimate ${CACHED_PREFIX_TOKEN_ESTIMATE} outside [3800, 6750]`);
});

// ═════════════════════════════════════════════════════════
// 🛑 6/26 fable 審查 (Vivi 審過) — 紅線21 疲乏豁免 + 核心句式一主三變
// ═════════════════════════════════════════════════════════
test('🛑 6/26 (Vivi 審過): 紅線21 疲乏豁免 + 純見證輪 (只暫停問句收尾、危機優先)', () => {
  const damonCore = CACHED_PREFIX_SECTIONS[0];
  assert.match(damonCore.content, /疲乏豁免/);
  assert.match(damonCore.content, /純見證輪/);
  assert.match(damonCore.content, /只暫停「問句收尾」、不暫停其他紅線/);
  assert.match(damonCore.content, /以紅線 20 優先處理/);
});

test('🛑 6/26 (Vivi 審過): 核心句式一主三變 + 連續 ≤2 次 (主引擎保留)', () => {
  const damonCore = CACHED_PREFIX_SECTIONS[0];
  assert.match(damonCore.content, /擁有這個，對你有什麼重要/);          // 主引擎保留
  assert.match(damonCore.content, /這個底下、還有什麼更重要的東西/);
  assert.match(damonCore.content, /你會是一個怎麼樣不一樣的人/);
  assert.match(damonCore.content, /它在幫你守住什麼/);
  assert.match(damonCore.content, /同一句式連續使用 ≤2 次/);
});
...
test('🛑 fable 6/26: E1c 首次明確退場 → 先給選擇權(Vivi 核准軟化)', () => {
  const c = ENGINE_1_PIPELINE.sub_prompts.E1c.prompt_content;
  assert.match(c, /首次明確退場請求/);
  assert.match(c, /我們可以停在這裡,或者再走一小步/);
  assert.match(c, /尊重真實退場/);
  // 既有挑戰內容仍在(只是退場請求先給選擇權、不移除挑戰路徑)
  assert.match(c, /我不喜歡這個答案/);
});

...
test('cached: damon-core-philosophy includes 20 紅線 (§3 patch 5/21)', () => {
  const damonCore = CACHED_PREFIX_SECTIONS[0];
  assert.match(damonCore.content, /20 條紅線/);
  assert.equal(/16 條紅線/.test(damonCore.content), false, 'title must say 20, not 16');

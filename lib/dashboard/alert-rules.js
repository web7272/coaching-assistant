...
// ⭐ v5.2 第五塊 — case 3 cross-context swap escalate (spec §4.2「連續 3 turn」).
//   chat.js tracks session_state.cross_context_swap_count (++ on swap intent,
//   reset 0 on non-swap turn). >= threshold → HITL Vivi (學員 context 設錯
//   或真的想換 → 評估後台改 active_context).
export const CROSS_CONTEXT_SWAP_THRESHOLD = Object.freeze({
  consecutive_turns: 3,
});
...
export function evaluateModeThrash({ recentSessionTransitionCounts = [] } = {}) {
  ...
  return {
    ...
    action: 'HITL 觀察(連續 3 sessions → Vivi 主動聯絡, errata v02 不再 offer 1-on-1 預約)',
  };
}
...
/**
 * Rule 7 (v5.2 第五塊) — cross_context_swap_escalate_alert.
 *
 * Source: v52_context_anchored_spec §4.2 — student verbally requests context
 * swap >= 3 consecutive turns → escalate Vivi for evaluation (either 後台
 * active_context 設錯, or 學員真的想換領域 → Vivi 決定 PATCH students).
 *
 * Severity: HITL (Vivi review, not critical). Dispatch wire-up (email/Brevo)
 * is the existing gap — this rule emits structured alert + log only.
 *
 * @param {object} args
 * @param {number} args.cross_context_swap_count - session_state.cross_context_swap_count
 * @param {number} [args.active_context_category] - optional, for context in payload
 * @returns {object|null}
 */
export function evaluateCrossContextSwapEscalate({
  cross_context_swap_count = 0,
  active_context_category = null,
} = {}) {
  const n = Number(cross_context_swap_count);
  if (!Number.isFinite(n) || n < CROSS_CONTEXT_SWAP_THRESHOLD.consecutive_turns) return null;
  return {
    type: 'cross_context_swap_escalate_alert',
    severity: ALERT_SEVERITY.HITL,
    reason: `student verbally requested context swap ${n} consecutive turns (threshold ${CROSS_CONTEXT_SWAP_THRESHOLD.consecutive_turns})`,
    payload: {
      consecutive_turns: n,
      threshold: CROSS_CONTEXT_SWAP_THRESHOLD.consecutive_turns,
      active_context_category,
    },
    source_spec: 'v52_context_anchored_spec §4.2 case 3 連續 3 turn → escalate Vivi',
    action: 'HITL Vivi: 評估 學員是否需要 PATCH active_context (後台改) — 鐵律 #2 dispatch wire-up 仍是既有缺口、本批落 log + flag',
  };
}
...
export function evaluateAllRules(state = {}) {
  ...
  // v5.2 第五塊 — case 3 swap escalate (additive; older callers omit the inputs → null).
  push(evaluateCrossContextSwapEscalate({
    cross_context_swap_count: state.cross_context_swap_count,
    active_context_category: state.active_context_category,
  }));
  return alerts;
}

...
import {
  ...
  evaluateCrossContextSwapEscalate,
...
test('🛑 evaluateAllRules: integrates all 6 rules', () => {
  ...
  // All 6 original alerts triggered (cross-context-swap not provided → 0).
...
// ─── Rule 7 (v5.2 第五塊) — cross_context_swap_escalate ─────

test('🛑 CROSS_CONTEXT_SWAP_THRESHOLD: 連續 3 turn (spec §4.2)', () => {
  assert.equal(CROSS_CONTEXT_SWAP_THRESHOLD.consecutive_turns, 3);
});

test('🛑 evaluateCrossContextSwapEscalate: count < 3 → null', () => {
  assert.equal(evaluateCrossContextSwapEscalate({ cross_context_swap_count: 0 }), null);
  assert.equal(evaluateCrossContextSwapEscalate({ cross_context_swap_count: 1 }), null);
  assert.equal(evaluateCrossContextSwapEscalate({ cross_context_swap_count: 2 }), null);
});

test('🛑 evaluateCrossContextSwapEscalate: count >= 3 → HITL alert', () => {
  const a = evaluateCrossContextSwapEscalate({
    cross_context_swap_count: 3, active_context_category: 2,
  });
  assert.ok(a);
  assert.equal(a.type, 'cross_context_swap_escalate_alert');
  assert.equal(a.severity, ALERT_SEVERITY.HITL);
  assert.equal(a.payload.consecutive_turns, 3);
  assert.equal(a.payload.threshold, 3);
  assert.equal(a.payload.active_context_category, 2);
  assert.match(a.source_spec, /v52_context_anchored_spec §4.2/);
});

test('🛑 evaluateCrossContextSwapEscalate: count = 5 still HITL (monotonic over threshold)', () => {
  const a = evaluateCrossContextSwapEscalate({ cross_context_swap_count: 5 });
  assert.equal(a.severity, ALERT_SEVERITY.HITL);
  assert.equal(a.payload.consecutive_turns, 5);
});

test('🛑 evaluateCrossContextSwapEscalate: defensive — NaN / negative / non-number → null', () => {
  assert.equal(evaluateCrossContextSwapEscalate({}), null);
  assert.equal(evaluateCrossContextSwapEscalate({ cross_context_swap_count: 'foo' }), null);
  assert.equal(evaluateCrossContextSwapEscalate({ cross_context_swap_count: -3 }), null);
});

test('🛑 evaluateAllRules: cross_context_swap_count >= 3 → 7th alert', () => {
  setAlertLogger(() => {});
  const alerts = evaluateAllRules({
    cross_context_swap_count: 3,
    active_context_category: 1,
  });
  // Only cross-context swap fires when other inputs are empty.
  assert.equal(alerts.length, 1);
  assert.equal(alerts[0].type, 'cross_context_swap_escalate_alert');
  setAlertLogger(null);
});

...
import {
  ...
  addToSeminarList,
  createSeminarList,
  // 8/13 Vivi — daily signup report (每日 21:00 台灣 給 Vivi/Terry/support) 3 個 export
...
test('🛑 7/30 buildSeminarConfirmationHtml: Vivi 定稿 verbatim (3 行內文 + 主旨語意)', () => {
  ...
  // 2 行內文 (Vivi 8/14 verbatim, 全形標點, 刪「先想一件事」段)
  ...
  // 反向鎖:無舊品牌「21天身分重塑計畫」 / 無「——」破折號 / 無「先想一件事」段 (Vivi 8/14 刪)
  assert.doesNotMatch(html, /21天身分重塑計畫/);
  assert.equal(/——/.test(html), false, '無「——」破折號');
  assert.doesNotMatch(html, /先想一件事/, '8/14 Vivi 拍板刪 — 表單已收 question, 再問等於沒讀');
  assert.doesNotMatch(html, /你最近最想問自己的一個問題/);
});
...
test('🛑 buildSeminarConfirmationText: mirror (verbatim 2 行內文, 無 HTML tag, 無破折號, 無「先想一件事」段)', () => {
  ...
  assert.doesNotMatch(t, /先想一件事/, '8/14 Vivi 拍板刪');
});

// ─── sendSeminarConfirmation (mailer) ────────────────────

test('🛑 sendSeminarConfirmation: no key → stub + envMissing (Vivi 8/14: subscribe 聚合 log 用 envMissing 判 stub)', async () => {
  const r = await sendSeminarConfirmation('a@b.co', SEMINAR_THANKS_URL);
  assert.equal(r.ok, true);
  assert.equal(r.stubbed, true);
  assert.equal(r.envMissing, true, 'envMissing 旗標必須有, 區分「env 缺」vs「呼叫失敗」');
  assert.match(r.reason, /BREVO_API_KEY/);
});
...
test('🛑 sendSeminarConfirmation: with key → POST /v3/smtp/email (subject / sender / textContent)', async () => {
  ...
  // 8/14 Vivi: seminar 系列 sender 改 seminar@ (不再是 help@),
  // 對齊 thanks 頁 + daily-report 三方一致.
  assert.equal(b.sender.email, 'seminar@seeyourself.now');
...
test('🛑 sendSeminarConfirmation: 4xx/throw → stub 但無 envMissing (Vivi 8/14: 4xx 是呼叫失敗, 不是 env 缺)', async () => {
  process.env.BREVO_API_KEY = 'fake-key';
  _setFetchFn(async () => ({ ok: false, status: 429, text: async () => 'rate' }));
  let r = await sendSeminarConfirmation('a@b.co', SEMINAR_THANKS_URL);
  assert.equal(r.stubbed, true);
  assert.equal(r.envMissing, undefined, '4xx 是 runtime failure 不是 env missing');
  assert.match(r.reason, /429/);
  _setFetchFn(async () => { throw new Error('ECONNRESET'); });
  r = await sendSeminarConfirmation('a@b.co', SEMINAR_THANKS_URL);
  assert.equal(r.stubbed, true);
  assert.equal(r.envMissing, undefined, 'throw 是 runtime failure 不是 env missing');
  assert.match(r.reason, /ECONNRESET/);
});

// ─── addToSeminarList (POST /v3/contacts) ────────────────

test('🛑 addToSeminarList: no key → stub + envMissing', async () => {
...
test('🛑 addToSeminarList: no SEMINAR_LIST_ID → stub + envMissing', async () => {
  process.env.BREVO_API_KEY = 'fake-key';
...
test('🛑 addToSeminarList: bad SEMINAR_LIST_ID (non-numeric) → stub + envMissing', async () => {
  process.env.BREVO_API_KEY = 'fake-key';
...
// ═════════════════════════════════════════════════════════
// 8/13 Vivi — daily-signup-report (每日 21:00 台灣 快報)
// ═════════════════════════════════════════════════════════

const DAILY_DATA_EMPTY = { total: 27, todayCount: 0, todayItems: [], sourceBreakdown: [] };
const DAILY_DATA_FULL = {
  total: 27,
  todayCount: 2,
  todayItems: [
    { question: '為什麼我總是討好別人？', source: 'hero' },
    { question: '我到底想要什麼？',     source: 'signup' },
  ],
  sourceBreakdown: [
    { source: 'hero',   n: 18 },
    { source: 'signup', n:  9 },
...
// ─── buildDailyReportHtml (pure) ─────────────────────────

test('🛑 buildDailyReportHtml: total + todayCount 顯示正確 + masthead 對齊 v11', () => {
  const html = buildDailyReportHtml(DAILY_DATA_FULL);
  // Masthead 對齊 v11 (magic-link / guide / seminar) 古銅金 verbatim
  assert.match(html, /color:#B58F5C;font-weight:700;">看見自己/);
  assert.match(html, /SEE YOURSELF/);
  // Eyebrow「問對問題 · 9/30 每日快報」
  assert.match(html, /問對問題 · 9\/30 每日快報/);
  // 累計 27 + 今日 +2
  assert.match(html, /27 <span[^>]*>人累計<\/span>/);
  assert.match(html, /今日 \+2/);
});

test('🛑 buildDailyReportHtml: 今天的問題 逐條列出, escape 避免 XSS', () => {
  const html = buildDailyReportHtml({
    ...DAILY_DATA_FULL,
    todayItems: [{ question: '<script>alert(1)</script>', source: 'hero' }],
    todayCount: 1,
  });
  assert.match(html, /&lt;script&gt;alert\(1\)&lt;\/script&gt;/);
  // Ensure raw <script> did NOT leak.
  assert.equal(/[^&]<script>alert/.test(html), false);
});

test('🛑 buildDailyReportHtml: 0 人日子 顯示「今天 0 位」(心跳訊號)', () => {
  const html = buildDailyReportHtml(DAILY_DATA_EMPTY);
  assert.match(html, /今天 0 位。/);
  assert.match(html, /今日 \+0/);
});

test('🛑 buildDailyReportHtml: 內文不含 email (Vivi 決定: 降低外流風險)', () => {
  const html = buildDailyReportHtml({
    ...DAILY_DATA_FULL,
    todayItems: [{ question: '例題', source: 'hero' }],
    todayCount: 1,
  });
  // question / source 有, 但整個 template 不能出現 @ 或 email pattern
  assert.equal(/@/.test(html), false, '不能出現 @ (內文絕不列 email)');
});

test('🛑 buildDailyReportHtml: 來源分佈 sourceBreakdown 顯示', () => {
  const html = buildDailyReportHtml(DAILY_DATA_FULL);
  assert.match(html, /來源分佈 · 累計/);
  assert.match(html, /hero · <b[^>]*>18<\/b>/);
  assert.match(html, /signup · <b[^>]*>9<\/b>/);
});

test('🛑 buildDailyReportHtml: 無 sourceBreakdown → 該區塊不出現', () => {
  const html = buildDailyReportHtml({ ...DAILY_DATA_EMPTY, sourceBreakdown: [] });
  assert.equal(/來源分佈/.test(html), false);
});

// ─── buildDailyReportText (mirror) ───────────────────────

test('🛑 buildDailyReportText: 鏡像, 無 HTML tag, 無 email', () => {
  const t = buildDailyReportText(DAILY_DATA_FULL);
  assert.match(t, /^看見自己\n/);
  assert.match(t, /累計 27 人 · 今日 \+2/);
  assert.match(t, /1\. 為什麼我總是討好別人？/);
  assert.match(t, /2\. 我到底想要什麼？/);
  assert.match(t, /hero · 18/);
  assert.match(t, /看見自己團隊 · 自動快報$/);
  assert.equal(/<[a-zA-Z/!][^>]*>/.test(t), false, '無 HTML tag');
  assert.equal(/@/.test(t), false, '無 email');
});

test('🛑 buildDailyReportText: 0 人日子「今天 0 位」', () => {
  const t = buildDailyReportText(DAILY_DATA_EMPTY);
  assert.match(t, /今天 0 位。/);
});
...
test('🛑 sendDailyReport: no recipients → ok:false with reason', async () => {
  const r = await sendDailyReport({ ...REPORT_ARGS(), recipients: [] });
  assert.equal(r.ok, false);
  assert.match(r.reason, /no recipients/);
});

test('🛑 sendDailyReport: no key → stub (ok:true 對齊 sendMagicLink dev pattern)', async () => {
  const r = await sendDailyReport(REPORT_ARGS());
  assert.equal(r.ok, true);
  assert.equal(r.stubbed, true);
  assert.match(r.reason, /BREVO_API_KEY/);
});

test('🛑 sendDailyReport: with key → POST shape (sender=seminar@, multiple to)', async () => {
  process.env.BREVO_API_KEY = 'fake-key';
  const cap = [];
  _setFetchFn(async (u, i) => { cap.push({ u, i }); return { ok: true, status: 201, text: async () => '' }; });
  const r = await sendDailyReport(REPORT_ARGS());
  ...
  assert.equal(cap[0].u, 'https://api.brevo.com/v3/smtp/email');
  const b = JSON.parse(cap[0].i.body);
  assert.equal(b.sender.email, 'seminar@seeyourself.now', 'sender 必須是 seminar@ (不是 help@)');
  assert.equal(b.sender.name, '看見自己');
  assert.deepEqual(b.to, [{ email: 'a@b.co' }, { email: 'c@d.co' }]);
  assert.equal(b.subject, '報名累計 27 人');
});

test('🛑 sendDailyReport: 4xx/throw → ok:false (內部信要看得到失敗)', async () => {
  process.env.BREVO_API_KEY = 'fake-key';
  _setFetchFn(async () => ({ ok: false, status: 400, text: async () => 'sender not verified' }));
  let r = await sendDailyReport(REPORT_ARGS());
  assert.equal(r.ok, false);
  assert.match(r.reason, /400/);
  assert.match(r.detail, /sender not verified/);
  _setFetchFn(async () => { throw new Error('ECONNRESET'); });
  r = await sendDailyReport(REPORT_ARGS());
  assert.equal(r.ok, false);
  assert.match(r.reason, /ECONNRESET/);
});

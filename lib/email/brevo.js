...
export async function sendSeminarConfirmation(email, thanksUrl) {
  if (!process.env.BREVO_API_KEY) {
    // Vivi 8/14: env-missing 上 error 級, 讓 Vercel dashboard filter Error
    //   直接抓到 (Vivi 診斷: 上一 bug 就因為 warn 被 Vivi 漏看). Runtime
    //   network / 4xx 保持原 error 級, 只環境設定失誤這條上升.
    // eslint-disable-next-line no-console
...
export async function addToSeminarList(email, attrs = {}) {
  if (!process.env.BREVO_API_KEY) {
    // Vivi 8/14: env-missing → error 級 (見 sendSeminarConfirmation 註).
    // eslint-disable-next-line no-console

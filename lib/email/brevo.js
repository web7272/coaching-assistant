...
export async function sendSeminarConfirmation(email, thanksUrl) {
  if (!process.env.BREVO_API_KEY) {
    // Vivi 8/14: env-missing 上 error 級 + envMissing 旗標. subscribe.js 聚合
    //   log 用 envMissing 精確判斷 (別把 Brevo 4xx 也算 stub 誤導 Vivi).
    // eslint-disable-next-line no-console
    console.error(`[brevo:CRITICAL] seminar-confirm ${email}: BREVO_API_KEY missing — email NOT sent`);
    return { ok: true, stubbed: true, envMissing: true, reason: 'BREVO_API_KEY not configured' };
  }
...
export async function addToSeminarList(email, attrs = {}) {
  if (!process.env.BREVO_API_KEY) {
    // Vivi 8/14: env-missing → error 級 + envMissing 旗標 (見 sendSeminarConfirmation 註).
    // eslint-disable-next-line no-console
    console.error(`[brevo:CRITICAL] seminar-list add ${email}: BREVO_API_KEY missing — contact NOT added`);
    return { ok: true, stubbed: true, envMissing: true, reason: 'BREVO_API_KEY not configured' };
  }
  ...
  if (!Number.isFinite(listId) || listId <= 0) {
    // eslint-disable-next-line no-console
    console.error(`[brevo:CRITICAL] seminar-list add ${email}: SEMINAR_LIST_ID missing (${listIdRaw ?? 'unset'}) — contact NOT added`);
    return { ok: true, stubbed: true, envMissing: true, reason: 'SEMINAR_LIST_ID not configured' };
  }

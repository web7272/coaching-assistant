...
test('🛑 PR-J5 ①: view-note has #/storyboard entry link', () => {
  ...
  assert.match(sec, /<a [^>]*href="#\/storyboard"[^>]*>看看你的人生旅途/);
});

test('🛑 PR-J5 ①: view-note storyboard link uses hint-italic style (沿用既有卡內連結)', () => {
  const sec = sectionOf('view-note');
  // The anchor pointing at #/storyboard must carry the hint-italic class.
  assert.match(sec, /<a [^>]*href="#\/storyboard"[^>]*class="[^"]*hint-italic[^"]*"|<a [^>]*class="[^"]*hint-italic[^"]*"[^>]*href="#\/storyboard"/);
});

test('🛑 PR-J5 ①: view-note storyboard link sits AFTER btn-row (not above it)', () => {
  const sec = sectionOf('view-note');
  const btnRowIdx = sec.indexOf('note-btn-row');
  const linkIdx   = sec.indexOf('#/storyboard');
  assert.ok(btnRowIdx > 0, 'btn-row anchor must exist');
  assert.ok(linkIdx > 0,   'storyboard link must exist');
  assert.ok(btnRowIdx < linkIdx,
    'storyboard link must follow btn-row (visually below the main CTAs)');
});
...
test('🛑 PR-J5 ②: mini-map uses map SVG icon (not 皇冠), aria-label set', () => {
  ...
  assert.match(block, /aria-label="打開我的人生旅途"/);
...
test('🛑 PR-J5 ③: auth.html preview lists all 5 step labels in order', () => {
  ...
  const ol = olMatch[0];
  const labels = ['地圖頁', '對話', '教練筆記', '歡迎頁', '我的人生旅途'];
  let prevIdx = -1;
...
test('🛑 PR-J5 ③ VERBATIM: 白話說明 byte-identical to Patrick spec', () => {
  // Patrick spec逐字 (single line, full-width punctuation):
  //   「這整套是透過『對話』,帶你走過 7 步的身分轉變;這趟轉變旅程,會同步呈現在『我的人生旅途』。」
  // Stored as-is (no surrounding quote marks) in the page intro.
...
test('🛑 PR-J5 ③: auth.html steps ①-④ use placeholder frames (圖未掛載)', () => {
  // 4 placeholder frames for steps ①②③④ (assets not yet loaded).
  const placeholders = authHtml.match(/auth-preview__frame--placeholder/g) || [];
  assert.equal(placeholders.length, 4,
    'expect 4 placeholder frames for steps ①-④ (icons not yet mounted)');
});
...
test('🛑 PR-J5 ③ ANTI-REGRESSION: no auto-POST at load (P0 6/7 safety preserved)', () => {
  ...
  // Sanity check: count fetch occurrences in auth.html — must be exactly 1.
  const fetches = authHtml.match(/fetch\(/g) || [];
  assert.equal(fetches.length, 1,
    'auth.html must have EXACTLY 1 fetch call (inside button click handler)');
});
...
test('🛑 PR-J5 ③ CSS: .auth-preview rule exists + 5-column grid', () => {
  ...
  assert.match(appCss, /\.auth-preview__steps\s*\{[\s\S]*?grid-template-columns:\s*repeat\(5,/);
});

...
test('🛑 PR-J5 ③ VERBATIM: 白話說明 byte-identical to Vivi 6/12 改字版', () => {
  ...
  assert.match(authHtml,
    /這整套系統是透過『對話』,帶你走過 7 步驟的身分轉變;這趟轉變旅程,會同步呈現在『我的人生旅途』裡。/);
...
test('🛑 6/12: auth.html steps ①-④ use --photo <img> with correct src mapping', () => {
  ...
  assert.equal(photos.length, 4,
    '①-④ MUST use --photo class (4 webp screenshots mounted)');
  ...
  const ol = olMatch[0];
  const expected = [
    { step: '①', file: '21day.webp', label: '地圖頁' },
    { step: '②', file: 'chat.webp',  label: '對話' },
    { step: '③', file: 'coach.webp', label: '教練筆記' },
    { step: '④', file: 'login.webp', label: '歡迎頁' },
  ];

...
test('🛑 PR-J5 ①: view-note has #/storyboard entry link', () => {
  ...
  assert.match(sec, /<a [^>]*href="#\/storyboard"[^>]*aria-label="我的故事"/);
});
...
test('🛑 6/26 Vivi: auth 預覽 = 單張 21天總攬縮圖 (4 卡 → 21 格 overview)', () => {
  assert.match(authHtml, /class="auth-preview__overview"/);
  const tiles = authHtml.match(/class="apo-tile"/g) || [];
  assert.equal(tiles.length, 21, '21天總攬縮圖要 21 格');
  assert.equal(/auth-preview__frame--photo/.test(authHtml), false, '舊 4 卡 photo 已移除');
  assert.equal(/<ol class="auth-preview__steps">/.test(authHtml), false, '舊 4 卡 ol 已移除');
});

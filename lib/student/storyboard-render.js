...
// ─── Patrick 6/26 — 我的故事 PDF (P2 列印視圖, pure render) ───────────
// 七步點亮時間軸:每步從 history(每日快照)找「第一天被填上」的 day.
export function renderProgressTimelineHTML(history = {}) {
  const h = (history && typeof history === 'object') ? history : {};
  const rows = STORYBOARD_STEP_META.map((meta) => {
    const stepKey = 'step_' + parseInt(meta.no, 10);
    let firstDay = null;
    for (let day = 1; day <= 21; day++) {
      const snap = h['day_' + day];
      const st = snap && snap[stepKey];
      const filled = !!(st && ((st.brain_state && st.brain_state.description) || st.sovereign_action));
      if (filled) { firstDay = day; break; }
    }
    return { no: meta.no, name_zh: meta.name_zh, firstDay };
  });
  const items = rows.map((r) => {
    const tag = r.firstDay ? `第 ${r.firstDay} 天點亮` : '尚未';
    const cls = r.firstDay ? 'pdf-tl-item pdf-tl-item--lit' : 'pdf-tl-item';
    return `<li class="${cls}"><span class="pdf-tl-no">${escapeHTML(r.no)}</span>`
      + `<span class="pdf-tl-name">${escapeHTML(r.name_zh)}</span>`
      + `<span class="pdf-tl-day">${escapeHTML(tag)}</span></li>`;
  }).join('');
  return `<ol class="pdf-timeline">${items}</ol>`;
}

const BOOK_QUILL_SVG = '<svg viewBox="0 0 24 24" width="44" height="44" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 8.3C9.2 6.4 5.8 6.1 2.8 6.8V16.9"/><path d="M2.8 16.9C5.6 16.2 9.2 16.5 12 18.5"/><path d="M12 8.3C14.8 6.4 18.2 6.1 21.2 6.8V16.9"/><path d="M21.2 16.9C18.7 16.3 15.7 16.4 13.2 17.7"/><path d="M12 8.3V18.5"/><path d="M19.8 4.4C17.8 6.6 16.6 9.6 16.3 12.8"/><path d="M19.8 4.4C20.2 6.8 19.7 9.2 18.2 11.2"/><path d="M16.3 12.8C17.2 12.4 17.9 11.9 18.2 11.2"/><path d="M16.3 12.8L15.2 14.8"/></svg>';

// 整份「我的故事」PDF 組版. data = GET /api/storyboard-pdf-data 回傳.
export function renderStoryPdfHTML(data = {}) {
  const d = (data && typeof data === 'object') ? data : {};
  if (!d.ready) {
    return `<div class="pdf-notready">完整的「我的故事」會在你走完 21 天、結業之後開放下載。</div>`;
  }
  const name = escapeHTML(d.name || '你');
  const grad = (d.graduation && typeof d.graduation === 'object') ? d.graduation : {};
  const values = (d.values && typeof d.values === 'object') ? d.values : {};
  const cards = Array.isArray(d.dailyCards) ? d.dailyCards : [];

  // 封面
  const cover = `<section class="pdf-cover">
    <div class="pdf-cover__logo">${BOOK_QUILL_SVG}</div>
    <h1 class="pdf-cover__title">${name} 的身分重塑之旅</h1>
    <p class="pdf-cover__sub">21 天身分重塑計畫 · 我的故事</p>
  </section>`;

  // 宣言
  const decl = grad.declaration
    ? `<section class="pdf-section pdf-section--decl"><p class="pdf-decl">${escapeHTML(grad.declaration)}</p></section>`
    : '';

  // 教練見證信 (可能多段, \n 分段)
  const letter = grad.coachLetter
    ? `<section class="pdf-section"><h2 class="pdf-h2">教練見證信</h2>`
      + escapeHTML(grad.coachLetter).split(/\n+/).map(p => `<p class="pdf-p">${p}</p>`).join('')
      + `<p class="pdf-sig">— 教練</p></section>`
    : '';

  // 7 步我的故事 (主體, 重用 storyboard body, 全展開)
  const story = `<section class="pdf-section pdf-page-break"><h2 class="pdf-h2">我的故事</h2>`
    + renderStoryboardBodyHTML({ steps: d.storyboard }, { collapsible: false, expandAllSteps: true })
    + `</section>`;

  // 21 天進度
  const timeline = `<section class="pdf-section pdf-page-break"><h2 class="pdf-h2">21 天進度</h2>`
    + `<p class="pdf-p pdf-muted">你的故事一天天長出來——每一步在第幾天被點亮:</p>`
    + renderProgressTimelineHTML(d.history)
    + `</section>`;

  // 價值觀
  const ranking = Array.isArray(values.ranking) ? values.ranking : [];
  const rankItems = ranking.slice(0, 10)
    .map(r => `<li>${escapeHTML(typeof r === 'string' ? r : (r && r.value) || '')}</li>`)
    .filter(x => !/>(<\/li>)/.test(x)).join('');
  const valuesSec = (values.top1 || rankItems)
    ? `<section class="pdf-section"><h2 class="pdf-h2">你的價值觀</h2>`
      + (values.top1 ? `<p class="pdf-p">最重要的:<strong>${escapeHTML(values.top1)}</strong></p>` : '')
      + (rankItems ? `<ol class="pdf-values">${rankItems}</ol>` : '')
      + `</section>`
    : '';

  // 每日身分解析卡 附錄
  const appendix = cards.length
    ? `<section class="pdf-section pdf-page-break"><h2 class="pdf-h2">每日身分解析卡</h2>`
      + cards.map(c => `<article class="pdf-card"><div class="pdf-card__day">第 ${escapeHTML(String(c.day))} 天</div>`
          + `<div class="pdf-card__body">${escapeHTML(c.card || '').split(/\n+/).map(p => `<p>${p}</p>`).join('')}</div></article>`).join('')
      + `</section>`
    : '';

  return `<div class="pdf-doc">${cover}${decl}${letter}${story}${timeline}${valuesSec}${appendix}</div>`;
}

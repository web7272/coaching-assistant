...
test('🛑 6/12 storylink consistency: 每個故事標籤 href 必須對得上 block id (防 SPA router fall-through)', () => {
  ...
  // Side-nav links also must hit real ids (already padded before this PR, sanity check).
  const navHrefs = [...html.matchAll(
    /<a class="storyboard-nav-item[^"]*"[^>]*href="(#storyboard-step-\d+)"/g)
  ].map(m => m[1]);
  assert.equal(navHrefs.length, 7, 'side nav must contain 7 links');
...
test('🛑 6/12 collapsible: 故事外層包 .storyboard-story-wrap--collapsed (預設收起)', () => {
  const html = renderStoryboardBodyHTML(null);
  assert.match(html, /class="storyboard-story-wrap storyboard-story-wrap--collapsed"/,
    'wrap must default to --collapsed modifier');
  // 內層 .storyboard-story id 仍在 (handler 依賴此 id).
  assert.match(html, /<div class="storyboard-story" id="storyboard-story">/);
});

test('🛑 6/12 collapsible: toggle 是 <button> + aria-expanded=false + 「展開更多」', () => {
  const html = renderStoryboardBodyHTML(null);
  // <button type="button"> a11y; 用 anchor 會被 SPA router 抓.
  assert.match(html, /<button type="button" class="storyboard-story-toggle"/);
  // 預設 collapsed → aria-expanded false.
  assert.match(html, /class="storyboard-story-toggle"[^>]*aria-expanded="false"/);
  // 對得到內層 story id.
  assert.match(html, /aria-controls="storyboard-story"/);
  // 預設 label.
  assert.match(html, />展開更多</);
});
...
test('🛑 6/12 coach opts: collapsible=false → 去掉 --collapsed modifier + 不放 toggle button', () => {
  ...
  assert.match(html, /class="storyboard-story-wrap"/);
  assert.equal(/storyboard-story-wrap--collapsed/.test(html), false,
    'coach mode: --collapsed modifier MUST be absent (故事直接全展開)');
  // Toggle button 完全沒出現 (coach 唯讀, 不需互動).
  assert.equal(/storyboard-story-toggle/.test(html), false,
...
test('🛑 6/12 coach opts: collapsible=true (default) 仍與舊行為一致 (有 --collapsed + toggle)', () => {
  ...
  assert.match(a, /storyboard-story-wrap--collapsed/);
  assert.match(a, /storyboard-story-toggle[^>]*aria-expanded="false"/);
  assert.match(b, /storyboard-story-wrap--collapsed/);
});
...
test('🛑 6/12 coach opts: expandAllSteps=true → 7 步 aria-expanded 全為 true (body 全不 hidden)', () => {
  ...
  // 沒任何 false (排除 storyboard-story-toggle, 但 collapsible=true 是 default 仍會放 toggle).
...
test('🛑 6/12 coach opts: full happy combo (collapsible=false + expandAllSteps=true) — review-friendly markup', () => {
  ...
  assert.equal(/storyboard-story-wrap--collapsed|storyboard-story-toggle/.test(html), false);
  // 7 步全展開.
...
test('🛑 6/12 sync-gate: student.js inlines collapsible wrap + toggle markup', () => {
  assert.ok(studentJsSrc.includes('storyboard-story-wrap--collapsed'),
    'student.js inline must wrap story in collapsible container (6/12)');
  assert.ok(studentJsSrc.includes('storyboard-story-toggle'),
    'student.js inline must include 展開更多 toggle button');
  assert.ok(studentJsSrc.includes('展開更多'),

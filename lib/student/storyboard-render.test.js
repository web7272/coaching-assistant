...
test('🛑 renderStepBlock: sovereign_action null → 主權宣告 section omitted', () => {
  const html = renderStepBlockHTML({
    no: '01', name_zh: '發現匱乏', name_en: 'The Void',
    state: 'filled',
  ...
  assert.match(html, /大腦現狀/);
  assert.equal(/主權宣告/.test(html), false);
});

test('🛑 S4 renderStepBlock: 步1–5 (note-based) → 大腦現狀 + 可能阻力 + 良善動機, 無主權宣告', () => {
  const html = renderStepBlockHTML({
  ...
  assert.match(html, /storyboard-section--brain/);
  assert.match(html, /大腦現狀/);
  assert.match(html, /storyboard-section--resistance/);
  assert.match(html, /可能阻力/);
  assert.match(html, /有一個聲音說/);
  assert.match(html, /storyboard-section--intent/);
  assert.match(html, /良善動機/);
  assert.match(html, /它保護你不必直視那個空/);
  // early step has no sovereign declaration section
  assert.equal(/主權宣告/.test(html), false);
});

test('🛑 S4 renderStepBlock: 步6–7 (climax) → 大腦現狀 + 主權宣告, 無阻力/動機', () => {
  const html = renderStepBlockHTML({
    no: '06', name_zh: '奪回主權', name_en: 'Reclaiming the Sovereignty', state: 'filled',
    brain_state: { description: '對方的選擇不是你的計分板。', quote: null },
    resistance: null, benevolent_intent: null,
    sovereign_action: '拿回那個開關。我說了算。',
  });
  assert.match(html, /主權宣告/);
  assert.match(html, /我說了算/);
  assert.equal(/可能阻力/.test(html), false);
  assert.equal(/良善動機/.test(html), false);
});

test('🛑 6/14 step toggle: 步驟頭右側「展開/收合」+ chevron, 取代英文名', () => {
  // 收起的步驟 (非 01) → 「展開」 + chevron, 英文名 span/文字都消失.
  const collapsed = renderStepBlockHTML({
    no: '02', name_zh: '承認渴望', name_en: 'The Longing',
    state: 'empty', brain_state: null, sovereign_action: null,
  });
  assert.match(collapsed, /storyboard-step-peek">展開</);
  assert.match(collapsed, /storyboard-step-chev/);
  assert.equal(/storyboard-step-name-en/.test(collapsed), false, '英文名 span 已移除');
  assert.equal(/The Longing/.test(collapsed), false, '英文名文字不再顯示');
  // 預設展開的 step 01 → 「收合」.
  const expanded = renderStepBlockHTML({
    no: '01', name_zh: '發現匱乏', name_en: 'The Void',
    state: 'empty', brain_state: null, sovereign_action: null,
  });
  assert.match(expanded, /storyboard-step-peek">收合</);
  assert.equal(/The Void/.test(expanded), false);
});

test('🛑 6/14 sync-gate: student.js 步驟頭鏡像 展開/收合 + chevron + 切換', () => {
  assert.ok(studentJsSrc.includes('storyboard-step-peek'), 'student.js 須鏡像 step-peek');
  assert.ok(studentJsSrc.includes('storyboard-step-chev'), 'student.js 須鏡像 step-chev');
  assert.equal(/storyboard-step-name-en/.test(studentJsSrc), false,
    'student.js 步驟頭英文名 span 已移除');
  // 點擊 handler 須切換 peek 文字.
  assert.match(studentJsSrc, /peek\.textContent = next \? '收合' : '展開'/,
    'student.js step toggle handler 須切換 peek 文字');
});

test('🛑 6/14 sync-gate: index.html 頁Y 頂部有「回到旅程」鈕 (data-nav=journey)', () => {
  // header 內 (在 storyboard-title 之前) 須有一顆 data-nav="journey" 的 paper-btn.
  const headerSlice = indexHtml.slice(
    indexHtml.indexOf('class="storyboard-header"'),
    indexHtml.indexOf('storyboard-title'));
  assert.match(headerSlice, /storyboard-topbar/);
  assert.match(headerSlice, /data-nav="journey"[^>]*>回到旅程|回到旅程/);
});
...
test('🛑 PR-J4 sync-gate: index.html storyboard header carries 「我的故事」 title', () => {
  const storyboardSection = indexHtml.match(
    /<section id="view-storyboard"[\s\S]*?<\/section>/);
  assert.ok(storyboardSection);
  assert.match(storyboardSection[0], /我的故事/);
});
...
// ─── Patrick 6/26 — 我的故事 PDF (P2) ───────────────────────────
test('🛑 PDF: 未結業 (ready:false) → 不吐故事、顯示開放說明', () => {
  const h = renderStoryPdfHTML({ ready: false });
  assert.match(h, /結業之後開放/);
  assert.equal(/pdf-cover/.test(h), false, '未結業不該有封面/內容');
});

test('🛑 PDF: ready → 含封面/宣言/見證信/我的故事/進度/價值觀/附錄', () => {
  const h = renderStoryPdfHTML({
    ready: true, name: 'Vivi',
    graduation: { declaration: '我決定我是誰', coachLetter: '第一段\n第二段' },
    storyboard: [],
    history: { day_3: { step_1: { brain_state: { description: 'x' } } } },
    values: { top1: '自由', ranking: ['自由', '勇敢'] },
    dailyCards: [{ day: 1, card: '卡片內容' }],
  });
  assert.match(h, /pdf-cover/);
  assert.match(h, /Vivi 的身分重塑之旅/);
  assert.match(h, /我決定我是誰/);
  assert.match(h, /教練見證信/);
  assert.match(h, /pdf-timeline/);
  assert.match(h, /你的價值觀/);
  assert.match(h, /每日身分解析卡/);
});

test('🛑 PDF timeline: 從 history 算每步第一天點亮', () => {
  const tl = renderProgressTimelineHTML({ day_3: { step_1: { sovereign_action: 'a' } } });
  assert.match(tl, /pdf-timeline/);
  assert.match(tl, /第 3 天點亮/);   // step_1 在 day_3 點亮
  assert.match(tl, /尚未/);          // 其他步尚未
});

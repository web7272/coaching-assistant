...
/**
 * 找今天的 session；無則建立新 row（跨 day reset 在此處 bake 進 session_state）。
 *
 * @returns {Promise<{sessionId:number, turnCount:number, sessionStart:Date,
 *                     sessionState:object, isNew:boolean}>}
 */
async function loadOrCreateSession(sql, { studentId, module, sessionDate, week, day, sessionNotes }) {
  const todayRows = await sql`
    SELECT id, questions_today, created_at, session_state
    FROM sessions
    WHERE student_id = ${studentId} AND module = ${module} AND session_date = ${sessionDate}
    ORDER BY created_at DESC
    LIMIT 1
  `;
  if (todayRows.length > 0) {
    return {
      sessionId:    todayRows[0].id,
      turnCount:    todayRows[0].questions_today || 0,
      sessionStart: new Date(todayRows[0].created_at),
      sessionState: todayRows[0].session_state || {},
      isNew:        false,
    };
  }

  // 無今日 session → 抓最近一個 prior session、carry phase 進度
  const priorRows = await sql`
    ...
    LIMIT 1
  `;
  const priorState = priorRows[0]?.session_state || null;
  const initialState = buildCarryOverState(priorState);

  const created = await sql`
    INSERT INTO sessions
      (student_id, module, week, day, session_date, session_notes, questions_today, session_state)
    VALUES
      (${studentId}, ${module}, ${week}, ${day}, ${sessionDate}, ${sessionNotes || ''}, 0,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Step 4 — user profile + new_session_day 偵測
    ...
    // Step 5 — load / create today's session（跨 day reset bake 在 buildCarryOverState）
    const sess = await loadOrCreateSession(sql, {
      studentId, module, sessionDate,
      week: parseInt(week) || 1, day: day || 1, sessionNotes,
    });
